    ...
    try {
      ...
      try {
        ...
        getLoggerInstance().warn(
          { error: githubError },
          'GitHub API unavailable, using cached data',
        );
  ...
  constructor() {
    ...
    try {
      ...
      if (rootPackageJson.repository?.url) {
        ...
        const match = rootPackageJson.repository.url.match(
          /github\.com[/:]([^/]+)\/([^/.]+)/,
        );
        if (match) {
      ...
      this.logger.warn(
        { error },
        'Could not determine repository from package.json',
      );
      this.githubRepo = 'Jboner-Corvus/AgenticForge'; // Fallback
    }
  }
...
describe('VersionService', () => {
  ...
  it('should handle GitHub API errors gracefully', async () => {
    ...
    await expect(versionService.getLatestRelease()).rejects.toThrow(
      'API Error',
    );
  });
});

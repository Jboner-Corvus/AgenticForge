      ...
      >
        ...
        <TabsContent value="console" className="flex-1 flex flex-col gap-4 p-4">
          <div className="flex-1 grid grid-rows-2 gap-4">
            ...
            />
            <OutputPanel results={results} onClear={() => setResults([])} />
          </div>
        </TabsContent>
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      switch (command) {
        ...
        case 'trace':
          jsCommand =
            'console.trace(); "Stack trace logged to browser console"';
          isDeveloperCommand = true;
          break;
        default:
      ...
      getLoggerInstance().error(
        { error },
        'Error executing enhanced client console command',
      );
...
router.get(
  '/api/client-console/history/:sessionId',
  async (req: express.Request, res: express.Response) => {
    try {
...
describe('Enhanced Client Console API', () => {
  it('should handle developer console commands', async () => {
    ...
    expect(response.body.status).toBe(
      'Enhanced command sent to client. Awaiting result...',
    );
  });

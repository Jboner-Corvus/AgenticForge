...
const todolistTests = {
  crud: [
    {
      id: 403,
      name: 'TodoWrite création',
      description: 'Créer nouvelle todo avec ID unique',
    },
    {
      id: 404,
      name: 'TodoWrite modification',
      description: 'Update statut et contenu',
    },
    {
      id: 405,
      name: 'TodoWrite suppression',
      description: 'Soft delete avec historique',
    },
    {
      id: 406,
      name: 'TodoList lecture',
      description: 'Pagination et tri par statut',
    },
    {
      id: 407,
      name: 'TodoList filtrage',
      description: 'Filter par pending/in_progress/completed',
    },
  ],
  ...
  statuts: [
    {
      id: 408,
      name: 'Transition pending→in_progress',
      description: 'Validation règles métier',
    },
    {
      id: 409,
      name: 'Transition in_progress→completed',
      description: 'Timestamp completion',
    },
    {
      id: 410,
      name: 'Transition completed→pending',
      description: 'Réouverture si nécessaire',
    },
    {
      id: 411,
      name: 'Statut in_progress unique',
      description: 'Une seule tâche active',
    },
    {
      id: 412,
      name: 'Validation transitions',
      description: 'Reject transitions invalides',
    },
  ],
  ...
  persistence: [
    {
      id: 413,
      name: 'Persistance Redis',
      description: 'Save/load état complet',
    },
    { id: 414, name: 'Sync temps réel', description: 'WebSocket updates' },
    {
      id: 415,
      name: 'Backup automatique',
      description: 'Snapshots périodiques',
    },
  ...
  agent: [
    ...
    {
      id: 419,
      name: 'Décomposition tâches',
      description: 'Sous-tâches logiques',
    },
    {
      id: 420,
      name: 'Priorité intelligente',
      description: 'Ordre optimal exécution',
    },
    {
      id: 421,
      name: 'Estimation temps',
      description: 'Durée prévue par tâche',
    },
    {
      id: 422,
      name: 'Gestion dépendances',
      description: 'Tâches bloquantes/bloquées',
    },
  ],
};
...
async function testTodoListSystem() {
  ...
  try {
    const createResponse = await fetch('http://localhost:3001/api/test-chat', {
      ...
      body: JSON.stringify({
        prompt:
          'Crée une todo liste avec ces tâches: tester Canvas, tester Playwright, valider intégration',
        sessionName: 'Test TodoList Creation',
      }),
    });

    console.log(
      `   ${createResponse.ok ? '✅' : '⚠️'} Agent todo creation (${createResponse.status})`,
    );
  } catch (error) {
  ...
  try {
    const updateResponse = await fetch('http://localhost:3001/api/test-chat', {
      ...
      body: JSON.stringify({
        prompt:
          'Marque la première tâche comme "in_progress" et la deuxième comme "completed"',
        sessionName: 'Test TodoList Update',
      }),
    });

    console.log(
      `   ${updateResponse.ok ? '✅' : '⚠️'} Agent todo update (${updateResponse.status})`,
    );
  } catch (error) {
    console.log(`   ❌ Error: ${error.message.split(' ')[0]}`);
  }

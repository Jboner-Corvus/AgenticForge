...
import { getErrDetails, WebhookError, type ErrorDetails } from './errorUtils.js';
...
export async function sendWebhook<P, R>(
  ...
  try {
    ...
    log.info(`Envoi du webhook avec signature... Payload size: ${JSON.stringify(payload).length}`);
    ...
    log.info(`Webhook envoyé avec succès. Status: ${res.status}`);
    return true;
  } catch (error: unknown) {
...
export function verifyWebhookSignature(
  ...
  try {
    ...
    logger.error(
      { err: getErrDetails(error as Error) },

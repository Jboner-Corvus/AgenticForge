// src/webServer.ts (Proxy final et fonctionnel)
...
app.use(['/mcp', '/health'], (req: Request, res: Response) => {
  const targetPath = req.originalUrl === '/mcp' ? '/' : req.originalUrl;
  const targetUrl = `${PROXY_TARGET}${targetPath}`;

  logger.info(
    { originalUrl: req.originalUrl, targetUrl: targetUrl },
    'Proxying request to server',
  );

...
async function checkWorkerStatus() {
  ...
  console.log('\n=== LLM Configuration ===');
  console.log(
    `LLM Provider: ${process.env.LLM_PROVIDER || config.LLM_PROVIDER}`,
  );
  console.log(
    `LLM Model: ${process.env.LLM_MODEL_NAME || config.LLM_MODEL_NAME}`,
  );
  ...
  console.log(
    '4. Consider reducing the complexity of prompts if responses are truncated',
  );
}

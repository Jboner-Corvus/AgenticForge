...
import { _internalLoadTools, getTools } from './packages/core/dist/utils/toolLoader.js';
...
async function testTeslaSupport() {
  ...
  try {
    ...
    const alphaVantageTools = Object.keys(tools).filter(name => 
      name.includes('alpha_vantage') || name.includes('global_quote') || name.includes('time_series')
    );
    ...
    if (tools.alpha_vantage_ping) {
      ...
      const pingResult = await tools.alpha_vantage_ping.execute({}, { log: console.log });

...
commands = [
    ...
    # 2
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "2. Crée un fichier nommé rapport.txtet écris dedans :Ceci est la première ligne."}\' "http://192.168.2.56:8080/api/chat"',
    # 3
    ...
    # 4
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "4. Dans rapport.txt, remplace premièreparseconde."}\' "http://192.168.2.56:8080/api/chat"',
    # 5
    ...
    # 10
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "10. Crée un nouvel outil nommé dire_bonjourqui ne prend aucun paramètre et retourne la chaîneBonjour le monde !."}\' "http://192.168.2.56:8080/api/chat"',
    # 11
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "11. Crée un fichier todo.txtavec trois lignes :- Tâche 1, - Tâche 2, - Tâche 3. Ensuite, lis le contenu de ce fichier pour confirmer."}\' "http://192.168.2.56:8080/api/chat"',
    # 12
    ...
    # 16
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "16. Crée un fichier config.jsonavec le contenu{\\"user\\": \\"admin\\", \\"version\\": \\"2.1.0\\", \\"active\\": true}. Ensuite, lis ce fichier et dis-moi uniquement la valeur de la clé version."}\' "http://192.168.2.56:8080/api/chat"',
    # 17
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "17. Crée une arborescence de projet dans le workspace : un dossier parent nommé webapp, et à l\'\'intérieur, deux sous-dossiers : srcetassets."}\' "http://192.168.2.56:8080/api/chat"',
    # 18
    ...
    # 19
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "19. Écris un script Python word_counter.pyqui ouvre et lit le fichierprojet_alpha.md, compte le nombre de mots, et affiche le total. Exécute ensuite ce script."}\' "http://192.168.2.56:8080/api/chat"',
    # 20
    ...
    # 24
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "24. Crée un fichier A.txt avec helloet un fichier B.txt avecworld. Lis les deux fichiers et dis-moi s\'\'ils ont le même contenu."}\' "http://192.168.2.56:8080/api/chat"',
    # 25
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "25. Utilise la commande tarpour créer une archiveworkspace.tar.gz de tout le contenu du répertoire de travail."}\' "http://192.168.2.56:8080/api/chat"',
    # 26
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "26. Crée un outil nommé get-timestampqui ne prend aucun paramètre et retourne la date et l\'\'heure actuelles au format ISO 8601. La fonction execute doit simplement contenirreturn new Date().toISOString();."}\' "http://192.168.2.56:8080/api/chat"',
    # 27
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "27. J\'\'ai besoin d\'\'un outil pour générer des UUID. Crée un outil nommé generate-uuidqui utilise la fonctioncrypto.randomUUID() de Node.js. Après sa création, utilise-le immédiatement pour générer un UUID et l\'\'afficher."}\' "http://192.168.2.56:8080/api/chat"',
    # 28
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "28. Crée un outil log-activityqui prend un paramètremessage. Cet outil doit utiliser writeFilepour ajouter le message précédé d\'\'un timestamp dans un fichieractivity.log. Crée cet outil."}\' "http://192.168.2.56:8080/api/chat"',
    # 29
    ...
    # 30
    'curl -v -X POST -H "Content-Type: application/json" -H "Authorization: Bearer Qp5brxkUkTbmWJHmdrGYUjfgNY1hT9WOxUmzpP77JU0" -d \'{"prompt": "30. Je dois souvent vérifier si des sites web sont en ligne. Crée un outil check-website-statusqui prend uneurlen paramètre. Cet outil doit utiliser Python et la librairierequests pour faire une requête GET à l\'\'URL et retourner \\"En ligne\\" si le code de statut est 200, et \\"Hors ligne\\" sinon. Crée cet outil pour moi."}\' "http://192.168.2.56:8080/api/chat"',
    # 31

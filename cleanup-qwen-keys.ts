...
async function cleanupQwenKeys() {
  try {
    ...
    keys.push({
      apiKey:
        '_trPxyNaN47_vdVlnVTBu3RkqdaccB6g0YzjaCc3kxjw7f6O7omPJKQZhQRcCCDxOJ5mZ8CeODz17v0t-fKRUw',
    ...
    console.log(
      'Cleaned up Qwen keys - kept only the valid one with correct baseUrl',
    );
    ...
    const updatedKeys: LlmApiKey[] = updatedKeysJson.map((key: string) =>
      JSON.parse(key),
    );
    const qwenKeysAfter = updatedKeys.filter(

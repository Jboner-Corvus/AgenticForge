...
show_main_help() {
    ...
    echo "  Comprehensive management script for AgenticForge development platform."
    echo "  Provides complete service management, testing, development tools, and deployment."
    echo ""
    ...
    echo "    check-workers            Check workers and auto-cleanup if > 1"
    echo ""
    echo -e "  ${HELP_COLOR_CYAN}💻 Development:${HELP_COLOR_NC}"
    echo "    dev                      Start development server on port 3003"
    ...
    echo "    rebuild-web              Rebuild frontend only (faster)"
    echo ""
    echo -e "  ${HELP_COLOR_CYAN}🧪 Testing & Quality:${HELP_COLOR_NC}"
    echo "    test:unit                Run unit tests only"
    echo "    test:integration         Run integration tests"
    echo "    test:all                 Run all tests (unit + integration)"
    echo "    quality-check            Run lint, typecheck, and unit tests"
    echo ""
    echo -e "  ${HELP_COLOR_CYAN}📦 Installation & Setup:${HELP_COLOR_NC}"
    echo "    install/deploy           Fully automated installation (no prompts)"
    echo "    setup                    Run interactive setup wizard"
    echo ""
    echo -e "  ${HELP_COLOR_CYAN}❓ Help & Interface:${HELP_COLOR_NC}"
    ...
    echo "  $0 dev                     # Start development server on port 3003"
    echo "  $0 test:all                # Run complete test suite"
    echo "  $0 rebuild-web             # Quick frontend rebuild"
    echo "  $0 check-workers           # Verify worker health"
    ...
    echo "  performance, environment, devserver"
    echo ""
...
show_testing_help() {
    ...
    echo "  • Check service status: ./run.sh status"
...
show_troubleshooting_help() {
    ...
    echo "   • Check logs: ./run.sh menu → option 8"
    echo ""
    echo -e "${HELP_COLOR_YELLOW}2. Build failures:${HELP_COLOR_NC}"
    echo "   • Clean rebuild: ./run.sh rebuild-all"
    ...
    echo "   • Verify services: ./run.sh status"
    echo "   • Check test logs: /tmp/agenticforge-tests/"
    echo "   • Run tests individually: cd packages/core && pnpm test"
    echo ""
    echo -e "${HELP_COLOR_YELLOW}4. Full rebuild:${HELP_COLOR_NC}"
    echo "   • Full rebuild: ./run.sh rebuild-all"
    echo ""
    echo -e "${HELP_COLOR_YELLOW}5. Database issues:${HELP_COLOR_NC}"
    echo "   • Check PostgreSQL: docker exec g_forge_db pg_isready"
    echo "   • Reset database: docker compose down -v && docker compose up -d"
    echo ""
    echo -e "${HELP_COLOR_YELLOW}6. Authentication errors (401):${HELP_COLOR_NC}"
...
show_development_help() {
    ...
    echo "   ./run.sh test:unit          # Verify installation"
    echo ""
    ...
    echo "   • Run tests: ./run.sh test:unit"
    echo ""
    echo -e "3. ${HELP_COLOR_GREEN}Before Committing:${HELP_COLOR_NC}"
    echo "   ./run.sh test:all           # Full test suite"
    ...
    echo "  • Check logs: ./run.sh menu → option 5 (Worker logs)"
...
show_deployment_help() {
    ...
    echo ""
    echo -e "□ Run full test suite: ${HELP_COLOR_GREEN}./run.sh test:all${HELP_COLOR_NC}"
...
show_devserver_help() {
    echo -e "${HELP_COLOR_CYAN}╔════════════════════════════════════════╗"
    echo -e "║        DEVELOPMENT SERVER HELP         ║"
    echo -e "╚════════════════════════════════════════╝${HELP_COLOR_NC}"
    echo ""
    echo -e "${HELP_COLOR_WHITE}DEVELOPMENT SERVER:${HELP_COLOR_NC}"
    echo "  The development server provides hot-reload functionality for"
    echo "  frontend development on port 3003, separate from production services."
    echo ""
    echo -e "${HELP_COLOR_WHITE}COMMANDS:${HELP_COLOR_NC}"
    echo ""
    echo -e "  ${HELP_COLOR_GREEN}dev${HELP_COLOR_NC}                      Start development server on port 3003"
    echo -e "  ${HELP_COLOR_RED}stop-dev${HELP_COLOR_NC}                 Stop development server"
    echo -e "  ${HELP_COLOR_BLUE}dev-logs${HELP_COLOR_NC}                 View development server logs"
    echo ""
    echo -e "${HELP_COLOR_WHITE}USAGE:${HELP_COLOR_NC}"
    echo "  ./run.sh dev                      # Start dev server"
    echo "  ./run.sh stop-dev                 # Stop dev server"
    echo "  ./run.sh dev-logs                 # View logs (Ctrl+C to exit)"
    echo ""
    echo -e "${HELP_COLOR_WHITE}FEATURES:${HELP_COLOR_NC}"
    echo "  • Hot reload on file changes"
    echo "  • Separate from production services"
    echo "  • Runs on port 3003"
    echo "  • React Fast Refresh enabled"
    echo "  • Source maps for debugging"
    echo ""
    echo -e "${HELP_COLOR_WHITE}WHEN TO USE:${HELP_COLOR_NC}"
    echo "  • Frontend development and testing"
    echo "  • UI/UX prototyping"
    echo "  • Quick iterations on React components"
    echo "  • Debugging frontend issues"
    echo ""
    echo -e "${HELP_COLOR_WHITE}DIFFERENCES FROM PRODUCTION:${HELP_COLOR_NC}"
    echo "  • Development server (port 3003) vs Production (port 3002)"
    echo "  • Hot reload vs static builds"
    echo "  • Development dependencies included"
    echo "  • Source maps and debugging tools"
    echo ""
    echo -e "${HELP_COLOR_WHITE}LOG FILES:${HELP_COLOR_NC}"
    echo "  Development server logs: ./dev-server.log"
    echo "  Production server logs: docker compose logs web"
    echo ""
    echo -e "${HELP_COLOR_WHITE}TROUBLESHOOTING:${HELP_COLOR_NC}"
    echo "  • Port 3003 in use: ./run.sh stop-dev"
    echo "  • Build errors: Check ./dev-server.log"
    echo "  • Dependencies: pnpm install in packages/ui/"
    echo "  • Clear cache: rm -rf packages/ui/node_modules/.vite"
}
...
show_contextual_help() {
    ...
    case "$context" in
        ...
        "test_failure")
            ...
            echo "1. Check service status: ./run.sh status"
        ...
        "build_error")
            ...
            echo "4. Full rebuild: ./run.sh rebuild-all"
            ;;
        *)
            show_main_help
            ;;

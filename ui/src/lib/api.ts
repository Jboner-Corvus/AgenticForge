 ...
 * @returns {Promise<unknown>} La réponse du serveur (contenant le jobId).
 */
export async function sendMessage(prompt: string): Promise<unknown> {
  try {
 ...
 */
export async function getHistory(): Promise<Array<unknown>> {
  try {
 ...
 */
export async function getTools(): Promise<Array<unknown>> {
  try {

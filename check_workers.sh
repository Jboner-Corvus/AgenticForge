...
# Get worker process details
get_worker_details() {
    local pattern="$WORKER_PATTERN"

    if command_exists pgrep; then
        # Use pgrep with ps for detailed info
        ...
        # Fallback to ps with grep
        ps aux | grep -E "$pattern" | grep -v grep | \
            awk '{print $2 " " $3 " " $4 " " $11 " " $12 " " $13}'
    fi
}
...
check_worker_health() {
    ...
    # Check if process is responsive (basic health check)
    if timeout 5s kill -USR1 "$worker_pid" 2>/dev/null; then

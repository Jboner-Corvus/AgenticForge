...
const tradingTests = {
  alphaVantage: [
    {
      ...
      description: 'Données intraday 1min-60min',
    },
    {
      id: 494,
      api: 'TIME_SERIES_DAILY',
      description: 'Données quotidiennes historiques',
    },
  ...
  technicalAnalysis: [
    {
      id: 520,
      feature: 'Support/Résistance',
      description: 'Détection automatique pivot points',
    },
    {
      id: 521,
      feature: 'Fibonacci',
      description: 'Retracements 23.6%, 38.2%, 61.8%',
    },
    {
      id: 532,
      feature: 'Corrélations',
      description: 'Actions secteur tech/finance',
    },
    {
      id: 536,
      feature: 'Heatmap',
      description: 'Matrix corrélations temps réel',
    },
    {
      id: 538,
      feature: 'VIX Analysis',
      description: 'Fear index market sentiment',
    },
  ],
  ...
  canvasCharts: [
    {
      id: 544,
      chart: 'Candlestick',
      description: 'OHLC avec volumes, zoom/pan',
    },
    {
      id: 549,
      chart: 'Indicators Overlay',
      description: 'SMA, EMA, Bollinger Bands',
    },
    { id: 550, chart: 'Oscillators', description: 'RSI, MACD subplots' },
    {
      id: 559,
      chart: 'Correlation Heatmap',
      description: 'Matrix interactive drill-down',
    },
    {
      id: 561,
      chart: 'Portfolio Pie',
      description: 'Allocation avec performance',
    },
  ],
  ...
  agentTrader: [
    { id: 564, strategy: 'Momentum', description: 'MACD + RSI signals' },
    {
      ...
      description: 'Bollinger Bands oversold/bought',
    },
    { id: 569, risk: 'Stop-Loss', description: 'ATR-based trailing stops' },
    {
      id: 574,
      analysis: 'Sentiment',
      description: 'News + social media aggregation',
    },
    {
      id: 584,
      backtest: 'Strategy',
...
async function testAlphaVantageAPIs() {
  ...
  for (const test of tradingTests.alphaVantage) {
    ...
    try {
      ...
      console.log(
        `   ${response.ok ? '✅' : '⚠️'} Agent response (${response.status})`,
      );
      ...
      if (test.api === 'GLOBAL_QUOTE') {
        ...
        if (directData['Global Quote']) {
          console.log(
            `   ✅ Direct API: Price ${directData['Global Quote']['05. price']}`,
          );
        } else {
          console.log(
            `   ⚠️ Direct API: ${directData.Information || 'Rate limit'}`,
          );
        }
      }
    } catch (error) {
...
async function testTechnicalAnalysis() {
  ...
  for (const test of tradingTests.technicalAnalysis) {
    ...
    try {
      ...
      console.log(
        `   ${response.ok ? '✅' : '⚠️'} Technical analysis (${response.status})`,
      );
    } catch (error) {
      console.log(`   ❌ Error: ${error.message.split(' ')[0]}`);
    }
...
async function testCanvasCharts() {
  ...
  for (const test of tradingTests.canvasCharts) {
    ...
    try {
      ...
      console.log(
        `   ${response.ok ? '✅' : '⚠️'} Canvas chart (${response.status})`,
      );
    } catch (error) {
      console.log(`   ❌ Error: ${error.message.split(' ')[0]}`);
    }
...
async function testAgentTrader() {
  ...
  for (const test of tradingTests.agentTrader) {
    console.log(
      `🧠 Test ${test.id}: ${test.strategy || test.risk || test.analysis || test.backtest}`,
    );
    console.log(`   Description: ${test.description}`);
    ...
    try {
      ...
      console.log(
        `   ${response.ok ? '✅' : '⚠️'} Agent trader (${response.status})`,
      );
    } catch (error) {
      console.log(`   ❌ Error: ${error.message.split(' ')[0]}`);
    }

...
describe('Trading Tools Integration', () => {
  it('should have trading tools directory', async () => {
    ...
    const { stdout } = await execa('ls', ['-la', 'tests/unit/tools/definitions/trading/'], {
      cwd: process.cwd(),
      reject: false
    });
  ...
  it('should have executable trading test script', async () => {
    ...
    const { stdout } = await execa('ls', ['-la', 'tests/unit/tools/definitions/trading/trading-tools-test.sh'], {
      cwd: process.cwd(),
      reject: false
    });

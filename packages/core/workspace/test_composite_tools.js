...
async function testCompositeTools() {
  ...
  const categories = {
    finance: tools.filter(
      (t) => t.name.includes('finance') || t.name.includes('global_quote'),
    ),
    file: tools.filter(
      (t) =>
    ...
    web: tools.filter(
      (t) => t.name.includes('web') || t.name.includes('playwright'),
    ),
    system: tools.filter(
      (t) =>
        t.name.includes('finish') ||
        t.name.includes('todo') ||
        t.name.includes('display'),
    ),

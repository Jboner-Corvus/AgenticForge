...
const DigitalCurrencyDailyParams = AlphaVantageBaseParams.extend({
  ...
  market: z
    ...
    .transform((val) => val.toUpperCase())
    .describe(
      'Market currency code to price the digital currency in (3-letter ISO code, e.g., USD, EUR, CNY)',
    ),
});
    ...
    execute: async (params, context) => {
      ...
      try {
        ...
        const apiParams: Record<string, string> = {
          ...
          market: parsedParams.market,
          apikey:
            parsedParams.apikey || getConfig().ALPHA_VANTAGE_API_KEY || '',
        };
        ...
        log.error(
          ...
          'Error fetching daily digital currency data',
        );
        throw new Error(

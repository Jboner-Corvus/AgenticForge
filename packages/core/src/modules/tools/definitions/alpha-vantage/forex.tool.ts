...
const ForexParams = z.object({
  ...
  from_symbol: z
    ...
    .optional()
    .describe(
      'From currency symbol (e.g., USD, EUR, GBP) - required for FX functions',
    ),
  to_symbol: z
    .string()
    .min(3)
    ...
    .optional()
    .describe(
      'To currency symbol (e.g., USD, EUR, GBP) - required for FX functions',
    ),
  from_currency: z
    .string()
    .optional()
    .describe(
      'From currency for exchange rate (physical or crypto) - required for CURRENCY_EXCHANGE_RATE',
    ),
  to_currency: z
    .string()
    .optional()
    .describe(
      'To currency for exchange rate (physical or crypto) - required for CURRENCY_EXCHANGE_RATE',
    ),
  interval: z
    .enum(['1min', '5min', '15min', '30min', '60min'])
    .optional()
    .describe('Time interval - required for FX_INTRADAY'),
  outputsize: z
    .enum(['compact', 'full'])
    .optional()
    .default('compact')
...
export const forexTool: Tool<typeof ForexParams> = {
  description:
    'Comprehensive foreign exchange (Forex) data including realtime exchange rates, intraday, daily, weekly, and monthly time series for currency pairs. Supports both physical currencies and cryptocurrencies.',
    ...
    try {
      ...
      switch (parsedParams.function) {
        case 'CURRENCY_EXCHANGE_RATE':
          ...
          if (!parsedParams.from_currency)
            throw new Error(
              'from_currency is required for CURRENCY_EXCHANGE_RATE',
            );
          if (!parsedParams.to_currency)
            throw new Error(
              'to_currency is required for CURRENCY_EXCHANGE_RATE',
            );
        ...
        case 'FX_INTRADAY':
          ...
          if (!parsedParams.from_symbol)
            throw new Error('from_symbol is required for FX_INTRADAY');
          if (!parsedParams.to_symbol)
            throw new Error('to_symbol is required for FX_INTRADAY');
          if (!parsedParams.interval)
            throw new Error('interval is required for FX_INTRADAY');
        ...
        case 'FX_DAILY':
          ...
          if (!parsedParams.from_symbol)
            throw new Error('from_symbol is required for FX_DAILY');
          if (!parsedParams.to_symbol)
            throw new Error('to_symbol is required for FX_DAILY');
          apiParams.from_symbol = parsedParams.from_symbol;
          apiParams.to_symbol = parsedParams.to_symbol;
          if (parsedParams.outputsize)
            apiParams.outputsize = parsedParams.outputsize;
        ...
        case 'FX_WEEKLY':
          ...
          if (!parsedParams.from_symbol)
            throw new Error('from_symbol is required for FX_WEEKLY');
          if (!parsedParams.to_symbol)
            throw new Error('to_symbol is required for FX_WEEKLY');
          apiParams.from_symbol = parsedParams.from_symbol;
          apiParams.to_symbol = parsedParams.to_symbol;
          if (parsedParams.datatype) apiParams.datatype = parsedParams.datatype;
          break;
        ...
        case 'FX_MONTHLY':
          ...
          if (!parsedParams.from_symbol)
            throw new Error('from_symbol is required for FX_MONTHLY');
          if (!parsedParams.to_symbol)
            throw new Error('to_symbol is required for FX_MONTHLY');
          apiParams.from_symbol = parsedParams.from_symbol;
          apiParams.to_symbol = parsedParams.to_symbol;
          if (parsedParams.datatype) apiParams.datatype = parsedParams.datatype;
          break;

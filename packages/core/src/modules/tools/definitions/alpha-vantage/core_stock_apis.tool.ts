...
const CoreStockParams = z.object({
  ...
  symbol: z.string().min(1).max(10).optional().describe('Stock symbol (e.g., IBM, AAPL) - required for most functions except SYMBOL_SEARCH and MARKET_STATUS'),
  keywords: z.string().optional().describe('Keywords to search for (e.g., "Microsoft", "MSFT") - required for SYMBOL_SEARCH'),
  interval: z.enum(['1min', '5min', '15min', '30min', '60min']).optional().describe('Time interval for data points - required for TIME_SERIES_INTRADAY'),
  ...
  adjusted: z.boolean().optional().default(true).describe('Whether to return adjusted data (true) or raw data (false) - for TIME_SERIES_INTRADAY'),
...
export const coreStockApisTool: Tool<typeof CoreStockParams> = {
  description: 'Comprehensive core stock market data APIs including time series (intraday, daily, weekly, monthly), quotes, bulk quotes, symbol search, and market status. Supports both raw and adjusted data with flexible output formats.',
  ...
  execute: async (params, context) => {
    ...
    try {
      log.info('Executing core stock API', {
        ...
        keywords: 'keywords' in parsedParams ? parsedParams.keywords : undefined
      });
      ...
      switch (parsedParams.function) {
        case 'TIME_SERIES_INTRADAY':
          ...
          if (!parsedParams.symbol) throw new Error('Symbol is required for TIME_SERIES_INTRADAY');
          if (!parsedParams.interval) throw new Error('Interval is required for TIME_SERIES_INTRADAY');
          ...
          if (parsedParams.adjusted !== undefined) apiParams.adjusted = parsedParams.adjusted.toString();
          if (parsedParams.extended_hours !== undefined) apiParams.extended_hours = parsedParams.extended_hours.toString();
          if (parsedParams.month) apiParams.month = parsedParams.month;
          if (parsedParams.entitlement) apiParams.entitlement = parsedParams.entitlement;
          break;

        case 'TIME_SERIES_DAILY':
          apiParams.function = 'TIME_SERIES_DAILY';
          if (!parsedParams.symbol) throw new Error('Symbol is required for TIME_SERIES_DAILY');
          apiParams.symbol = parsedParams.symbol;
          if (parsedParams.outputsize) apiParams.outputsize = parsedParams.outputsize;
          if (parsedParams.datatype) apiParams.datatype = parsedParams.datatype;
          if (parsedParams.entitlement) apiParams.entitlement = parsedParams.entitlement;
        ...
        case 'TIME_SERIES_DAILY_ADJUSTED':
          ...
          if (!parsedParams.symbol) throw new Error('Symbol is required for TIME_SERIES_DAILY_ADJUSTED');
          apiParams.symbol = parsedParams.symbol;
          if (parsedParams.outputsize) apiParams.outputsize = parsedParams.outputsize;
          if (parsedParams.datatype) apiParams.datatype = parsedParams.datatype;
          break;
        ...
        case 'TIME_SERIES_WEEKLY':
          ...
          if (!parsedParams.symbol) throw new Error('Symbol is required for TIME_SERIES_WEEKLY');
          apiParams.symbol = parsedParams.symbol;
          if (parsedParams.datatype) apiParams.datatype = parsedParams.datatype;
          break;
        ...
        case 'TIME_SERIES_WEEKLY_ADJUSTED':
          ...
          if (!parsedParams.symbol) throw new Error('Symbol is required for TIME_SERIES_WEEKLY_ADJUSTED');
          apiParams.symbol = parsedParams.symbol;
          if (parsedParams.datatype) apiParams.datatype = parsedParams.datatype;
          break;
        ...
        case 'TIME_SERIES_MONTHLY':
          ...
          if (!parsedParams.symbol) throw new Error('Symbol is required for TIME_SERIES_MONTHLY');
          apiParams.symbol = parsedParams.symbol;
          if (parsedParams.datatype) apiParams.datatype = parsedParams.datatype;
          break;
        ...
        case 'TIME_SERIES_MONTHLY_ADJUSTED':
          ...
          if (!parsedParams.symbol) throw new Error('Symbol is required for TIME_SERIES_MONTHLY_ADJUSTED');
          apiParams.symbol = parsedParams.symbol;
          if (parsedParams.datatype) apiParams.datatype = parsedParams.datatype;
          break;
        ...
        case 'GLOBAL_QUOTE':
          ...
          if (!parsedParams.symbol) throw new Error('Symbol is required for GLOBAL_QUOTE');
          apiParams.symbol = parsedParams.symbol;
          if (parsedParams.datatype) apiParams.datatype = parsedParams.datatype;
          if (parsedParams.entitlement) apiParams.entitlement = parsedParams.entitlement;
          break;
        ...
        case 'REALTIME_BULK_QUOTES':
          ...
          if (!parsedParams.symbol) throw new Error('Symbol is required for REALTIME_BULK_QUOTES');
          apiParams.symbol = parsedParams.symbol;
          if (parsedParams.datatype) apiParams.datatype = parsedParams.datatype;
          break;
        ...
        case 'SYMBOL_SEARCH':
          ...
          if (!parsedParams.keywords) throw new Error('Keywords is required for SYMBOL_SEARCH');
      ...
      const data = await makeAlphaVantageRequest(parsedParams.function, apiParams, parsedParams.datatype || 'json');
      ...
      log.error({ err: error, params: parsedParams }, 'Error executing core stock API');
      throw new Error(

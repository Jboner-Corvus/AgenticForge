...
const GlobalQuoteParams = AlphaVantageBaseParams
  .merge(SymbolParam)
  .extend({
    entitlement: z
...
export const globalQuoteTool: Tool<typeof GlobalQuoteParams> = {
  description: 'Returns the latest price and volume information for the specified security',
  ...
  execute: async (params, context) => {
    ...
    try {
      ...
      log.error({ err: error, params: parsedParams }, 'Error fetching global quote data');
      throw new Error(

...
export async function createRecoveryPoint(
  ...
  try {
    ...
    await redis.setex(recoveryKey, 30 * 24 * 60 * 60, JSON.stringify(recoveryPoint));
    ...
    console.log(`[PERSISTENCE] Recovery point created for session ${sessionId}: ${description}`);
  } catch (error) {
    console.error(`[PERSISTENCE] Error creating recovery point for session ${sessionId}:`, error);
    throw error;
  }
}
...
export async function getRecoveryPoints(sessionId: string): Promise<RecoveryPoint[]> {
  try {
    ...
    console.error(`[PERSISTENCE] Error getting recovery points for session ${sessionId}:`, error);
    return [];
  }
}
...
export async function restoreFromRecoveryPoint(
  ...
  try {
    ...
    const pointStr = await redis.get(recoveryKey);
    if (!pointStr) {
      console.log(`[PERSISTENCE] Recovery point ${recoveryPointId} not found for session ${sessionId}`);
      return null;
    }
    
    const point: RecoveryPoint = JSON.parse(pointStr);
    console.log(`[PERSISTENCE] State restored from recovery point for session ${sessionId}: ${point.description}`);
    
    return point.state;
  } catch (error) {
    console.error(`[PERSISTENCE] Error restoring from recovery point for session ${sessionId}:`, error);
    return null;
  }
}
...
export async function isRecoveryNeeded(sessionId: string): Promise<boolean> {
  try {
    ...
    const isLikelyInterruption = hasInProgressTasks && timeSinceLastActivity > 60000;

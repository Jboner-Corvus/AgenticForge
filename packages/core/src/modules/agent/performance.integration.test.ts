...
vi.mock('../../utils/llmProvider.ts', () => ({
  getLlmProvider: () => ({
    getLlmResponse: vi.fn().mockResolvedValue('{"answer": "Performance test"}'),
  }),
}));
...
vi.mock('./responseSchema.ts', () => ({
  llmResponseSchema: {
    parse: vi.fn().mockReturnValue({ answer: 'Performance test' }),
  },
}));
...
describe('Memory and Performance Integration Tests', () => {
  ...
  describe('Memory Management', () => {
    it('should monitor memory usage during agent execution', async () => {
      ...
      expect(memoryAfter.heapUsed - memoryBaseline.heapUsed).toBeLessThan(
        100 * 1024 * 1024,
    ...
    it('should handle memory pressure gracefully', async () => {
      ...
      mockSessionData.history = largeData.map((content, i) => ({
        content,
        id: `${i}`,
        timestamp: Date.now(),
        type: 'user',
  ...
  describe('Resource Optimization', () => {
    it('should optimize conversation history storage', async () => {
      ...
      agent = new Agent(
        mockJob,
        { ...mockSessionData, history: largeHistory as any },
        getMockQueue(),
        [],
        'openai',
        mockSessionManager,

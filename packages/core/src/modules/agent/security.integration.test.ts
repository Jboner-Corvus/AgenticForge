...
vi.mock('./responseSchema.ts', () => ({
  llmResponseSchema: {
    parse: vi.fn().mockReturnValue({ answer: 'Security test' }),
  },
}));
...
describe('Security and Validation Integration Tests', () => {
  ...
  describe('Authentication and Authorization', () => {
    it('should enforce session-based access control', async () => {
      ...
      const restrictedAgent = new Agent(
        {
          ...mockJob,
          data: { action: 'admin', prompt: 'Restricted operation' },
        },
        restrictedSession,
        getMockQueue() as any,
        [],
  ...
  describe('Data Protection', () => {
    it('should redact sensitive information from logs', async () => {
      const sensitivePrompt =
        'My password is secret123 and my API key is sk-1234567890';
      ...
      expect(
        savedSession?.history?.some(
          (msg: any) =>
            msg.content?.includes('secret123') ||
            msg.content?.includes('sk-1234567890'),
        ),
      ).toBeFalsy();
    });
  ...
  describe('Rate Limiting and DOS Protection', () => {
    ...
    it('should detect and prevent abuse patterns', async () => {
      const repetitivePrompts = Array.from(
        { length: 10 },

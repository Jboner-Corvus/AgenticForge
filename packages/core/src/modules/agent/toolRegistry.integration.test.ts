...
describe('Tool Registry Integration Tests', () => {
  ...
  describe('Basic Tool Execution', () => {
    ...
    it('should handle tool execution with nested parameters', async () => {
      ...
      const complexParams = {
        data: [
          { id: 1, metadata: { category: 'A', score: 0.8 }, name: 'Item 1' },
  ...
  describe('Tool Chaining and Orchestration', () => {
    it('should execute multiple tools in sequence', async () => {
      ...
      mockLlmProvider.getLlmResponse
        ...
        .mockResolvedValueOnce(
          '{"command": {"name": "dataProcessor", "params": {"data": ["line1", "line2"], "operation": "count"}}}',
        )
        .mockResolvedValueOnce(
          '{"command": {"name": "writeFile", "params": {"path": "/data/output.txt", "content": "Processed: 2 lines"}}}',
        )
        .mockResolvedValueOnce(
      ...
      mockResponseSchema.parse
        ...
        .mockReturnValueOnce({
          command: {
            name: 'dataProcessor',
            params: { data: ['line1', 'line2'], operation: 'count' },
          },
        })
        .mockReturnValueOnce({
          command: {
            name: 'writeFile',
            params: { content: 'Processed: 2 lines', path: '/data/output.txt' },
          },
        })
        .mockReturnValueOnce({
          answer: 'Data processing completed successfully',
      ...
      expect(mockToolRegistry.execute).toHaveBeenNthCalledWith(
        2,
        'dataProcessor',
        { data: ['line1', 'line2'], operation: 'count' },
        expect.any(Object),
      );
      expect(mockToolRegistry.execute).toHaveBeenNthCalledWith(
        3,
        'writeFile',
        { content: 'Processed: 2 lines', path: '/data/output.txt' },
        expect.any(Object),
      );
    });
  ...
  describe('Tool Security and Sandboxing', () => {
    ...
    it('should validate tool parameters before execution', async () => {
      ...
      expect(mockToolRegistry.validate).toHaveBeenCalledWith(
        'writeFile',
        invalidParams,
      );
  ...
  describe('Tool Performance and Monitoring', () => {
    it('should monitor tool execution performance', async () => {
      ...
      (mockToolRegistry.monitorToolExecution as any).mockImplementation(
        (toolName: string, params: any, callback: any) => {
    ...
    it('should handle tool timeout gracefully', async () => {
      ...
      (mockToolRegistry.executeWithTimeout as any).mockImplementation(
        (toolName: string, params: any, timeout: number) => {
  ...
  describe('Tool Error Handling and Recovery', () => {
    ...
    it('should provide detailed error diagnostics', async () => {
      ...
      const errorResult = mockSessionData.history.find(
        (msg) =>
          msg.type === 'tool_result' &&
          typeof msg.result === 'string' &&
  ...
  describe('Dynamic Tool Discovery and Loading', () => {
    ...
    it('should hot-reload updated tool definitions', async () => {
      ...
      expect(mockToolRegistry.getToolDefinition).toHaveBeenCalledWith(
        'readFile',
      );
    });

...
describe('Agent Comprehensive Tests', () => {
  ...
  describe('Agent Main Loop', () => {
    ...
    it('should handle canvas output', async () => {
      ...
      expect(mockSessionData.history.some(msg => msg.type === 'agent_canvas_output')).toBe(true);
    });
    ...
    it('should handle finish tool signal', async () => {
      ...
      mockToolRegistry.execute.mockResolvedValue({ answer: 'Task completed' });
    ...
    it('should handle finish tool with invalid response', async () => {
      ...
      mockToolRegistry.execute.mockResolvedValue('Invalid response');

      const result = await agent.run();

      expect(result).toContain('Finish tool did not return a valid answer object');
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle tool execution errors', async () => {
      ...
      mockLlmProvider.getLlmResponse
        ...
        .mockResolvedValueOnce('{"answer": "Handled error"}');

      mockResponseSchema.parse
        .mockReturnValueOnce({ command: { name: 'testTool', params: {} } })
        .mockReturnValueOnce({ answer: 'Handled error' });
    ...
    it('should handle FinishToolSignal exception', async () => {
      ...
      mockToolRegistry.execute.mockRejectedValue(
        new FinishToolSignal('Task completed via signal')
      );

      const result = await agent.run();

      expect(result).toBe('Task completed via signal');
    });
    ...
    it('should reach maximum iterations', async () => {
      ...
      expect(result).toBe('Agent reached maximum iterations without a final answer.');
    });
  });
  ...
  describe('Message History Management', () => {
    it('should properly format message history for LLM', async () => {
      // Add various message types to history
      mockSessionData.history = [
        { type: 'user', content: 'Hello', id: '1', timestamp: Date.now() },
        { type: 'agent_response', content: 'Hi there', id: '2', timestamp: Date.now() },
        { type: 'tool_call', toolName: 'testTool', params: {}, id: '3', timestamp: Date.now() },
        { type: 'tool_result', toolName: 'testTool', result: 'success', id: '4', timestamp: Date.now() },

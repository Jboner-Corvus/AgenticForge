...
describe('Agent Conversation Integration Tests', () => {
  ...
  describe('Complete Conversation Flows', () => {
    it('should handle a simple question-answer conversation', async () => {
      ...
      (mockResponseSchema.parse as Mock).mockReturnValue({
        answer:
          'Hello! I can help you with file operations, web searches, and more.',
      });
    ...
    it('should handle research and analysis workflow', async () => {
      ...
      (mockLlmProvider.getLlmResponse as Mock)
        ...
        .mockResolvedValueOnce(
          '{"command": {"name": "webSearch", "params": {"query": "AI agent frameworks 2024"}}}',
        )
        .mockResolvedValueOnce(
          '{"thought": "Great! I found relevant information. Let me analyze and summarize it."}',
        )
        .mockResolvedValueOnce(
          '{"answer": "Based on my research, here are the top AI agent frameworks in 2024: 1) LangChain 2) AutoGPT 3) AgentGPT 4) CrewAI. Each has unique strengths for different use cases."}',
        );
      ...
      (mockResponseSchema.parse as Mock)
        ...
        .mockReturnValueOnce({
          command: {
            name: 'webSearch',
            params: { query: 'AI agent frameworks 2024' },
          },
        })
        .mockReturnValueOnce({
          thought:
            'Great! I found relevant information. Let me analyze and summarize it.',
        })
        .mockReturnValueOnce({
          answer:
            'Based on my research, here are the top AI agent frameworks in 2024: 1) LangChain 2) AutoGPT 3) AgentGPT 4) CrewAI. Each has unique strengths for different use cases.',
        });

      (mockToolRegistry.execute as Mock).mockResolvedValue({
        results: [
          {
            snippet: 'Framework for building LLM applications',
            title: 'LangChain Documentation',
          },
          { snippet: 'Autonomous AI agent framework', title: 'AutoGPT GitHub' },
          {
            snippet: 'Multi-agent collaboration framework',
    ...
    it('should handle canvas visualization workflow', async () => {
      (mockLlmProvider.getLlmResponse as Mock)
        ...
        .mockResolvedValueOnce(
          '{"canvas": {"content": "<div><h2>Sales Data</h2><canvas id=\"chart\"></canvas></div>", "contentType": "html"}}',
        );
      ...
      (mockResponseSchema.parse as Mock)
        ...
        .mockReturnValueOnce({
          canvas: {
            content:
              '<div><h2>Sales Data</h2><canvas id="chart"></canvas></div>',
            contentType: 'html',
          },
        });
      ...
      const canvasMessage = mockSessionData.history.find(
        (msg) => msg.type === 'agent_canvas_output',
      );
    ...
    it('should handle error recovery in conversation', async () => {
      ...
      (mockLlmProvider.getLlmResponse as Mock)
        ...
        .mockResolvedValueOnce(
          '{"thought": "The file doesn\'t exist. Let me try a different approach."}',
        )
        .mockResolvedValueOnce(
          '{"command": {"name": "webSearch", "params": {"query": "alternative data source"}}}',
        )
        .mockResolvedValueOnce(
          '{"answer": "I couldn\'t access the requested file, but I found alternative information online that might help."}',
        );
      ...
      (mockResponseSchema.parse as Mock)
        ...
        .mockReturnValueOnce({
          answer:
            "I couldn't access the requested file, but I found alternative information online that might help.",
        });
      ...
      (mockToolRegistry.execute as Mock)
        ...
        .mockResolvedValueOnce({
          results: [
            { snippet: 'Found relevant info', title: 'Alternative Data' },
          ],
        });
  ...
  describe('Complex Multi-Turn Conversations', () => {
    it('should maintain context across multiple interactions', async () => {
      ...
      mockSessionData.history = [
        {
          content: 'What is in my project folder?',
          id: '1',
          timestamp: Date.now() - 1000,
          type: 'user',
        },
        {
          content: 'I found 3 files: main.py, data.csv, and README.md',
          id: '2',
          timestamp: Date.now() - 900,
          type: 'agent_response',
        },
        {
          content: 'Can you analyze the data.csv file?',
          id: '3',
    ...
    it('should handle branching conversation paths', async () => {
      ...
      (mockLlmProvider.getLlmResponse as Mock)
        .mockResolvedValueOnce(
          '{"thought": "The user wants analysis. I should offer different types of analysis."}',
        )
        .mockResolvedValueOnce(
          '{"answer": "I can provide several types of analysis: 1) Statistical summary 2) Data visualization 3) Trend analysis. Which would you prefer?"}',
        );

      (mockResponseSchema.parse as Mock)
        .mockReturnValueOnce({
          thought:
            'The user wants analysis. I should offer different types of analysis.',
        })
        .mockReturnValueOnce({
          answer:
            'I can provide several types of analysis: 1) Statistical summary 2) Data visualization 3) Trend analysis. Which would you prefer?',
        });
    ...
    it('should handle clarification requests', async () => {
      ...
      (mockResponseSchema.parse as Mock).mockReturnValue({
        answer:
          "I'd be happy to help process a file! Could you please specify: 1) Which file you'd like me to process? 2) What type of processing you need (analysis, conversion, filtering, etc.)?",
      });
  ...
  describe('Agent Behavior Patterns', () => {
    it('should demonstrate reasoning and planning', async () => {
      (mockLlmProvider.getLlmResponse as Mock)
        ...
        .mockResolvedValueOnce(
          '{"command": {"name": "webSearch", "params": {"query": "research topic"}}}',
        )
        .mockResolvedValueOnce(
      ...
      (mockResponseSchema.parse as Mock)
        .mockReturnValueOnce({
          thought:
            'This is a complex request. I need to: 1) Search for information 2) Analyze the results 3) Create a summary report.',
        })
        .mockReturnValueOnce({
          command: { name: 'webSearch', params: { query: 'research topic' } },
        })
        .mockReturnValueOnce({
          thought:
            'Good results found. Now I need to analyze and structure this information.',
        })
        .mockReturnValueOnce({
          canvas: {
            content:
      ...
      (mockToolRegistry.execute as Mock).mockResolvedValue({
        results: [{ snippet: 'Important findings', title: 'Research Data' }],
      });
      ...
      const thoughts = mockSessionData.history.filter(
        (msg) => msg.type === 'agent_thought',
      );
      ...
      const explicitThoughts = thoughts.filter(
        (msg) => !msg.content.includes('The agent is thinking'),
      );
    ...
    it('should show adaptive behavior based on context', async () => {
      ...
      (mockResponseSchema.parse as Mock).mockReturnValue({
        answer:
          "Since you're working on data analysis in a development environment, I'll provide beginner-friendly explanations with each step.",
      });
    ...
    it('should handle tool chaining effectively', async () => {
      ...
      (mockLlmProvider.getLlmResponse as Mock)
        .mockResolvedValueOnce(
          '{"command": {"name": "readFile", "params": {"path": "input.txt"}}}',
        )
        .mockResolvedValueOnce(
          '{"thought": "I have the input data. Now I need to process it and save the results."}',
        )
        .mockResolvedValueOnce(
          '{"command": {"name": "writeFile", "params": {"path": "output.txt", "content": "Processed: [data]"}}}',
        )
        .mockResolvedValueOnce(
          '{"answer": "Successfully processed input.txt and saved results to output.txt"}',
        );
      ...
      (mockResponseSchema.parse as Mock)
        ...
        .mockReturnValueOnce({
          thought:
            'I have the input data. Now I need to process it and save the results.',
        })
        .mockReturnValueOnce({
          command: {
            name: 'writeFile',
            params: { content: 'Processed: [data]', path: 'output.txt' },
          },
        })
        .mockReturnValueOnce({
          answer:
            'Successfully processed input.txt and saved results to output.txt',
        });
  ...
  describe('Real-world Conversation Scenarios', () => {
    it('should handle technical support conversation', async () => {
      mockJob.data.prompt =
        'My application is not starting. Can you help debug?';

      (mockLlmProvider.getLlmResponse as Mock)
        .mockResolvedValueOnce(
          '{"thought": "User has an application startup issue. I should gather information about the error."}',
        )
        .mockResolvedValueOnce(
          '{"answer": "I\'ll help you debug the startup issue. Could you please: 1) Check the application logs 2) Verify your environment setup 3) Share any error messages you\'re seeing?"}',
        );

      (mockResponseSchema.parse as Mock)
        .mockReturnValueOnce({
          thought:
            'User has an application startup issue. I should gather information about the error.',
        })
        .mockReturnValueOnce({
          answer:
            "I'll help you debug the startup issue. Could you please: 1) Check the application logs 2) Verify your environment setup 3) Share any error messages you're seeing?",
        });
    ...
    it('should handle creative content generation', async () => {
      ...
      (mockResponseSchema.parse as Mock)
        .mockReturnValueOnce({
          thought:
            'The user wants creative content. I should write an engaging story.',
        })
        .mockReturnValueOnce({
          canvas: {
            content:
      ...
      const canvasOutput = mockSessionData.history.find(
        (msg) => msg.type === 'agent_canvas_output',
      );
    ...
    it('should handle data analysis workflow', async () => {
      ...
      (mockLlmProvider.getLlmResponse as Mock)
        .mockResolvedValueOnce(
          '{"thought": "I need to read the sales data first, then analyze it and create a comprehensive report."}',
        )
        .mockResolvedValueOnce(
          '{"command": {"name": "readFile", "params": {"path": "sales_data.csv"}}}',
        )
        .mockResolvedValueOnce(
          '{"thought": "Data loaded successfully. Now I\'ll analyze the trends and create a visual report."}',
        )
        .mockResolvedValueOnce(
          '{"canvas": {"content": "<div><h1>Sales Analysis Report</h1><p>Total Sales: $50,000</p><p>Top Product: Widget A</p></div>", "contentType": "html"}}',
        );

      (mockResponseSchema.parse as Mock)
        .mockReturnValueOnce({
          thought:
            'I need to read the sales data first, then analyze it and create a comprehensive report.',
        })
        .mockReturnValueOnce({
          command: { name: 'readFile', params: { path: 'sales_data.csv' } },
        })
        .mockReturnValueOnce({
          thought:
            "Data loaded successfully. Now I'll analyze the trends and create a visual report.",
        })
        .mockReturnValueOnce({
          canvas: {
            content:
              '<div><h1>Sales Analysis Report</h1><p>Total Sales: $50,000</p><p>Top Product: Widget A</p></div>',
            contentType: 'html',
          },
        });
  ...
  describe('Conversation Quality and Consistency', () => {
    ...
    it('should handle conversation transitions smoothly', async () => {
      ...
      (mockResponseSchema.parse as Mock).mockReturnValue({
        answer:
          "Of course! I'd be happy to switch topics and help you with cooking recipes. What type of cuisine or specific dish interests you?",
      });

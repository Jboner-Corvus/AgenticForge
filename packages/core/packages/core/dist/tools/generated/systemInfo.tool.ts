...
const systemInfoParams = z.object(
...
export const systemInfoTool = {
  name: 'system-info',
  description:
    '🤖 [OUTIL GÉNÉRÉ] Gathers system information including CPU usage, memory usage, and disk space',
  ...
  execute: async (args, ctx: Ctx) => {
    ...
    async function getMemoryUsageWindows() {
      try {
        const { stdout: totalMem } = await execPromise(
          'wmic ComputerSystem get TotalPhysicalMemory | findstr /R "[0-9]"',
        );
        const { stdout: freeMem } = await execPromise(
          'wmic OS get FreePhysicalMemory | findstr /R "[0-9]"',
        );
    ...
    async function getDiskUsageWindows() {
      try {
        const { stdout } = await execPromise(
          'wmic LogicalDisk where "DeviceID=\'C:\'" get Size,FreeSpace | findstr /R "[0-9]"',
        );

...
var MAX_TEMPORARY_ERROR_COUNT = 8;
var TEMPORARY_DISABLE_DURATION_MS = 5 * 60 * 1e3;
    ...
    try {
      ...
      if (masterKeyIndex !== -1) {
        ...
        if (existingMasterKey.isPermanentlyDisabled || existingMasterKey.isDisabledUntil && existingMasterKey.isDisabledUntil > Date.now()) {
          logger.info(
            { provider: existingMasterKey.apiProvider, apiKeyPrefix: existingMasterKey.apiKey.substring(0, 5) + "..." },
  ...
  */
  static async testAllKeys(dryRun = true) {
    ...
    try {
      ...
      reportLines.push(`\u{1F50D} Rapport de test de toutes les cl\xE9s (dryRun: ${dryRun})`);
      ...
      for (const key of keysToTest) {
        if (key.isPermanentlyDisabled) {
          reportLines.push(`\u23ED\uFE0F Cl\xE9 saut\xE9e (d\xE9sactiv\xE9e de mani\xE8re permanente): ${key.apiProvider} (${key.apiKey.substring(0, 5)}...)`);
          continue;
        }
        if (key.isDisabledUntil && key.isDisabledUntil > Date.now()) {
          const timeLeftSec = Math.ceil((key.isDisabledUntil - Date.now()) / 1e3);
          reportLines.push(`\u23ED\uFE0F Cl\xE9 saut\xE9e (d\xE9sactiv\xE9e temporairement, ${timeLeftSec}s restantes): ${key.apiProvider} (${key.apiKey.substring(0, 5)}...)`);
          continue;
        }
        testedKeys++;
        reportLines.push(`\u{1F9EA} Test de la cl\xE9: ${key.apiProvider} - ${key.apiModel} (${key.apiKey.substring(0, 5)}...)`);
        try {
          ...
          logger.warn({ err: testError, provider: key.apiProvider }, "Erreur non critique lors du test de la cl\xE9");
          if (!dryRun) {
          }
        }
      }
...
var QwenProvider = class {
  ...
  async getLlmResponse(messages, systemPrompt, apiKey, modelName) {
      ...
      try {
        if (attempt > 0) {
          if (attempt >= INITIAL_RETRIES_WITHOUT_DELAY) {
            const delayMs = 2e3 + (attempt - INITIAL_RETRIES_WITHOUT_DELAY) * 1e3;
            log.info(`Adding delay of ${delayMs}ms before Qwen API call (attempt ${attempt + 1})`);
            await new Promise((resolve) => setTimeout(resolve, delayMs));
          }
        } else {
          await new Promise(
            (resolve) => setTimeout(resolve, config.LLM_REQUEST_DELAY_MS)
          );
        }
        log.info(
          `[LLM CALL] Sending request to model: ${activeKey.apiModel} via ${activeKey.apiProvider} (attempt ${attempt + 1}/${MAX_RETRIES})`
        );
        ...
        getRedisClientInstance().incrby("leaderboard:tokensSaved", estimatedTokens).catch((error) => {
          getLogger().error(
            { error },
    ...
    if (lastError) {
      ...
      throw new LlmError2(`Failed to communicate with the LLM after ${MAX_RETRIES} attempts. Last error: ${lastError.message}`);
    }
    throw new LlmError2("Failed to communicate with the LLM.");
  }
};

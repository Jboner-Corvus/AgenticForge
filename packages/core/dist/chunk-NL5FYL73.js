...
var parameters = z.object({
  ...
  run_checks: z.boolean().optional().describe("Lancer automatiquement small-checks apr\xE8s cr\xE9ation (d\xE9faut: false)")
});
var GENERATED_TOOLS_DIR = path.resolve(process.cwd(), "packages/core/dist/tools/generated");
var TOOL_TEMPLATE = `
...
var createToolTool = {
  ...
  execute: async (args, ctx) => {
    const { description, execute_function, parameters: parameters2, tool_name, run_checks = false } = args;
    ...
    try {
      ...
      const testFileContent = TEST_TEMPLATE.replace(/\{\{tool_name\}\}/g, tool_name).replace(/\{\{toolVarName\}\}/g, toolVarName).replace(/\{\{toolFileName\}\}/g, toolFileName.replace(".ts", "")).replace(/\{\{description\}\}/g, description);

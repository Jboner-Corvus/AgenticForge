...
var GeminiProvider = class _GeminiProvider {
  ...
  async getLlmResponseWithRetry(messages, systemPrompt, apiKey, modelName, retryCount = 0) {
    ...
    const minDelay = getConfig().LLM_REQUEST_DELAY_MS || 3e3;
    ...
    try {
      ...
      if (content === void 0 || content === null || content.trim() === "") {
        log.error(
          { response: data },
      ...
      const isNetworkError = error.message.includes("AbortError") || error.message.includes("network") || error.message.includes("timeout") || error.message.includes("ECONNRESET") || error.message.includes("ETIMEDOUT") || error.message.includes("fetch failed") || error.message.includes("Failed to fetch") || error.name === "AbortError";
      const isRetryableApiError = error.message.includes("503") || error.message.includes("502") || error.message.includes("504") || error.message.includes("rate limit") || error.message.includes("temporarily unavailable") || error.message.includes("Internal server error");
      const isInvalidResponseError = error.message.includes("invalid response") || error.message.includes("parsing failed") || error.message.includes("JSON");
      if (isInvalidResponseError && retryCount < _GeminiProvider.MAX_RETRIES) {
        log.warn("Invalid response error detected - not retrying as this is likely a permanent issue");
      } else if ((isNetworkError || isRetryableApiError) && retryCount < _GeminiProvider.MAX_RETRIES) {
        const delay = _GeminiProvider.RETRY_DELAYS[Math.min(retryCount, _GeminiProvider.RETRY_DELAYS.length - 1)];
        const errorType = isNetworkError ? "network" : "API";
        log.info(`${errorType} error detected. Retrying in ${delay}ms... (attempt ${retryCount + 1}/${_GeminiProvider.MAX_RETRIES})`);

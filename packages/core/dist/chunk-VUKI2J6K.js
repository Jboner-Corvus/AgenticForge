...
var configSchema = z.object({
  AGENT_MAX_ITERATIONS: z.coerce.number().default(15),
  // 🚨 RÉDUIT: 100 -> 15 pour éviter les boucles infinies
  ...
  LLM_REQUEST_DELAY_MS: z.coerce.number().default(1e3),
  ...
  QWEN_API_BASE_URL: z.string().optional(),
  // Add Qwen base URL support
  ...
  // Reduced from 5 to 2 to prevent memory overflow
  ...
  GEMINI_MAX_HISTORY_LENGTH: z.coerce.number().default(50),
  // Limit history length for Gemini
  GEMINI_REQUEST_TIMEOUT_MS: z.coerce.number().default(3e4),
  // 30 second timeout for Gemini requests
  GEMINI_AUTO_REENABLE_KEYS: z.boolean().default(true),
  // Automatically re-enable temporarily disabled keys
  GEMINI_ITERATION_DELAY_MS: z.coerce.number().default(2e3),
  // Delay between iterations for Gemini
  AGENT_ITERATION_DELAY_MS: z.coerce.number().default(1e3),
  // Delay between iterations for other providers
  // Global timeout settings for robustness
  LLM_REQUEST_TIMEOUT_MS: z.coerce.number().default(45e3),
  // Global timeout for all LLM requests
  LLM_CONNECTION_TIMEOUT_MS: z.coerce.number().default(1e4),
  // Connection timeout
  LLM_MAX_RETRIES: z.coerce.number().default(3),
  // Maximum retry attempts for all providers
  LLM_RETRY_DELAY_BASE_MS: z.coerce.number().default(1e3),
  // Base delay for exponential backoff
  // Agent resilience settings
  AGENT_MAX_MALFORMED_RESPONSES: z.coerce.number().default(3),
  // 🚨 RÉDUIT: 5 -> 3 pour éviter les boucles
...
function getConfig() {
  if (!config2 || Object.keys(config2).length === 0) {
    try {
      ...
      if (!__require("fs").existsSync(envPath)) {

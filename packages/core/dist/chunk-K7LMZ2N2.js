...
import {
  ...
  getPostgresPool
} from "./chunk-2RTWIEDO.js";
import {
  SessionManager
} from "./chunk-5DBNVMWU.js";
import {
  summarizeTool
} from "./chunk-VF2SOJKI.js";
import {
  LlmError,
  getLlmProvider
...
var getSystemPromptContent = () => {
  ...
  const possiblePaths = [
    path.resolve(__dirname2, "system.prompt.md"),
    // dist/modules/agent/system.prompt.md
    path.resolve(__dirname2, "..", "..", "..", "system.prompt.md"),
    // dist/system.prompt.md
    path.resolve(__dirname2, "..", "..", "..", "..", "src", "modules", "agent", "system.prompt.md")
...
var Agent = class {
        ...
        try {
          const sessionWithContext = {
            data: {
              ...
              workingContext: {
                ...
                lastDisplayCanvas: this.hasExecutedActionRecently("display_canvas") ? `\u2705 display_canvas executed ${Math.floor((Date.now() - this.lastDisplayCanvasCall) / 1e3)}s ago` : "\u274C display_canvas not executed recently",
          ...
          for (let i = 0; i < config.LLM_PROVIDER_HIERARCHY.length; i++) {
            ...
            this.log.info({
              ...
              iteration: i
            }, "\u{1F50D} PROVIDER DEBUG: Provider selection details");
            try {
          ...
          if (canvas) {
            ...
            if (!command) {
              ...
              this.log.info("\u{1F527} Converting canvas output to display_canvas tool call");
            }
          }
          if (answer) {
            iterationLog.info({ answer }, "Agent final answer");
          ...
          if (errorMessage.includes("Failed to parse LLM response")) {
            ...
            continue;
          } else if (errorMessage.includes("Failed to communicate with the LLM") || errorMessage.includes("LLM API") || errorMessage.includes("network") || errorMessage.includes("timeout")) {
            ...
            await new Promise((resolve) => setTimeout(resolve, 2e3 * this.llmFailureCounter));
            continue;
          } else {
            this.session.history.push({
  ...
  convertPlainTextToValidJson(text) {
    ...
    if (toolCallMatch) {
      ...
      try {
        ...
        if (toolName === "display_canvas" && params && typeof params.content === "object") {
          this.log.info("\u{1F527} Converting display_canvas content object to JSON string");
    ...
    if (isTruncated) {
      ...
      const hasRecentTodoList = recentCommands.some(
        (cmd) => cmd.name === "todoWrite" && cmd.params && (cmd.params.action === "create" || cmd.params.action === "display")
      );
  ...
  parseLlmResponse(llmResponse, log) {
    ...
    const isIncomplete = llmResponse.trim().endsWith("...") || llmResponse.includes("ASSISTANT:") || llmResponse.includes("La r\xE9ponse de l'IA semble incompl\xE8te") || llmResponse.includes("The agent is thinking...") && !llmResponse.includes("Tool Call:") && !llmResponse.includes("{");
    const isRepetitive = this.detectRepetitiveResponse(llmResponse);
    if (isIncomplete) {
      log.warn("\u{1F6A8} R\xE9ponse LLM incompl\xE8te d\xE9tect\xE9e, for\xE7age fallback");
  ...
  trackExecutedAction(actionName, successful) {
    ...
    this.log.info(`\u{1F4CA} Action tracked: ${actionName} (success: ${successful}, total: ${(current?.count || 0) + 1})`);
  }
...
async function initializeWorker(redisConnection) {
  ...
  const sessionManager = await SessionManager.create(poolManager);
...
${finalMessage}`,
    ...
    },
    {
      ...
      // Now optimized for memory usage
  ...
  try {
    ...
    if (result === "OK") {
      ...
      const refreshInterval = setInterval(async () => {
        try {
          ...
          if (currentLock === processId) {
            ...
            getLoggerInstance().debug(`\u{1F504} Worker lock refreshed by process ${process.pid}`);
          } else {
      ...
      process.on("SIGTERM", async () => {
        ...
        getLoggerInstance().info(`\u{1F9F9} Worker lock released by process ${process.pid}`);
      });
...
if (process.env.NODE_ENV !== "test") {
  ...
  if (!canProceed) {
    getLoggerInstance().info("\u{1F6AB} [WORKER] Another worker is already running, exiting...");
    process.exit(0);
  }

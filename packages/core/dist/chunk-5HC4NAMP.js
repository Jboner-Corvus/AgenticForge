...
console.log("DEBUG: __dirname:", __dirname2);
...
var Agent = class {
        ...
        try {
          ...
          for (let i = 0; i < config.LLM_PROVIDER_HIERARCHY.length; i++) {
            ...
            try {
              ...
              if (llmError instanceof LlmError && providerToTry === "qwen" && (llmError.message.includes("Qwen API request failed with status 504") && llmError.message.includes("stream timeout") || llmError.message.includes("Qwen API request failed with status 502"))) {
                qwenTimeoutRetries++;
                this.log.warn(
  ...
  convertPlainTextToValidJson(text) {
    ...
    const todoKeywords = ["todo", "task", "list", "step", "plan", "workflow", "t\xE2che", "\xE9tape"];
    ...
    if (this.behaviorHistory.length >= this.loopDetectionThreshold) {
      if (command) {
        ...
        const allSameCommand = recentCommands.every(
          (behavior, index, arr) => behavior.command && behavior.command.name === command.name && JSON.stringify(behavior.command.params) === JSON.stringify(command.params)
        );
        if (allSameCommand) {
          this.log.warn(

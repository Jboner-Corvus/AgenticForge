...
// src/modules/api/canvasConsoleFeedback.api.ts
init_esm_shims();
...
var canvasConsoleFeedback_api_default = router2;

// src/modules/websocket/websocketServer.ts
init_esm_shims();
...
var WebSocketManager = class {
  ...
  constructor(server, redisClient) {
    ...
    logger.info("\u{1F4E1} WebSocket server initialized");
  }
  setupWebSocketServer() {
    this.wss.on("connection", (ws, req) => {
      ...
      logger.info(`\u{1F4E1} Client connected: ${clientId} (${this.clients.size} total clients)`);
    ...
    this.wss.on("error", (error) => {
      logger.error({ err: error }, "\u{1F4E1} WebSocket server error");
    });
  }
  setupClientListeners(client) {
    client.ws.on("message", (data) => {
      try {
        ...
        logger.error({ err: error, clientId: client.id }, "\u{1F4E1} Failed to parse WebSocket message");
      }
    });
    client.ws.on("close", (code, reason) => {
      logger.info(`\u{1F4E1} Client disconnected: ${client.id} (code: ${code}, reason: ${reason})`);
      this.handleClientDisconnect(client);
    });
    client.ws.on("error", (error) => {
      logger.error({ err: error, clientId: client.id }, "\u{1F4E1} WebSocket client error");
    });
    client.ws.on("pong", () => {
  ...
  handleClientMessage(client, message) {
    logger.info(`\u{1F4E1} Received message from client ${client.id}:`, message.type, message.data);
    switch (message.type) {
      case "ping":
        this.sendToClient(client, {
          type: "pong",
      ...
      case "subscribe_job_events":
        logger.info(`\u{1F4E1} Processing subscribe_job_events for job: ${message.data?.jobId}`);
        if (message.data?.jobId) {
          this.subscribeClientToJobEvents(client, message.data.jobId);
        } else {
          logger.warn(`\u{1F4E1} subscribe_job_events missing jobId from client ${client.id}`);
        }
        break;
      case "set_session":
        if (message.data?.sessionId) {
          client.sessionId = message.data.sessionId;
          logger.info(`\u{1F4E1} Client ${client.id} set session: ${client.sessionId}`);
        }
        break;
      default:
        logger.warn(`\u{1F4E1} Unknown message type: ${message.type} from client ${client.id}`);
    }
  }
  subscribeClientToJobEvents(client, jobId) {
    const channel = `job:${jobId}:events`;
    if (!client.subscribedChannels.has(channel)) {
      client.jobId = jobId;
      client.subscribedChannels.add(channel);
      this.redisSubscriber.subscribe(channel);
      logger.info(`\u{1F4E1} Client ${client.id} subscribed to job events: ${jobId}`);
      this.sendToClient(client, {
        type: "subscribed",
        data: { channel, jobId },
        timestamp: Date.now()
  ...
  handleClientDisconnect(client) {
    client.subscribedChannels.forEach((channel) => {
      const hasOtherSubscribers = Array.from(this.clients.values()).some((c) => c.id !== client.id && c.subscribedChannels.has(channel));
  ...
  setupRedisSubscriber() {
    this.redisSubscriber.on("message", (channel, message) => {
      try {
        ...
        logger.info(`\u{1F4E1} Redis message received on ${channel}:`, data.type);
        this.distributeMessage(channel, data);
      } catch (error) {
        logger.error({ err: error, channel }, "\u{1F4E1} Failed to parse Redis message");
      }
    });
    this.redisSubscriber.on("error", (error) => {
      logger.error({ err: error }, "\u{1F4E1} Redis subscriber error");
    });
    logger.info("\u{1F4E1} Redis subscriber initialized");
  }
  distributeMessage(channel, message) {
    const relevantClients = Array.from(this.clients.values()).filter((client) => client.subscribedChannels.has(channel));
    logger.info(`\u{1F4E1} Distributing message to ${relevantClients.length} clients on channel ${channel}`);
  ...
  sendToClient(client, message) {
    if (client.ws.readyState === WebSocket.OPEN) {
      try {
        ...
        logger.error({ err: error, clientId: client.id }, "\u{1F4E1} Failed to send message to client");
      }
    }
  }
  startPingInterval() {
    this.pingInterval = setInterval(() => {
      const now = Date.now();
      const pingTimeout = 6e4;
      this.clients.forEach((client) => {
        if (client.ws.readyState === WebSocket.OPEN) {
          try {
            client.ws.ping();
          } catch (error) {
            logger.error({ err: error, clientId: client.id }, "\u{1F4E1} Failed to ping client");
          }
          if (now - client.lastPing > pingTimeout) {
            logger.warn(`\u{1F4E1} Client ${client.id} ping timeout, closing connection`);
            client.ws.close(1e3, "Ping timeout");
          }
        }
      });
    }, 3e4);
  ...
  broadcastToJob(jobId, message) {
    const relevantClients = Array.from(this.clients.values()).filter((client) => client.jobId === jobId);
    relevantClients.forEach((client) => {
      this.sendToClient(client, message);
    });
  }
  ...
  async publishToChannel(channel, message) {
    try {
      ...
      logger.info(`\u{1F4E1} Published message to channel: ${channel}`);
    } catch (error) {
      logger.error({ err: error, channel }, "\u{1F4E1} Failed to publish to channel");
      throw error;
    }
  }
  close() {
    ...
    this.clients.forEach((client) => {
      if (client.ws.readyState === WebSocket.OPEN) {
        client.ws.close(1e3, "Server shutdown");
      }
    });
    this.wss.close();
    this.redisSubscriber.disconnect();
    logger.info("\u{1F4E1} WebSocket server closed");
  }
};
var websocketManager = null;
function initializeWebSocketManager(server, redisClient) {
  if (websocketManager) {
    logger.warn("\u{1F4E1} WebSocket manager already initialized");
...
async function initializeWebServer(pgClient, redisClient) {
  ...
  try {
    ...
    app.use(express3.static(uiDistPath));
    ...
    app.use(canvasConsoleFeedback_api_default);
    ...
    app.get("/api/canvas/assets/:jobId/:filename", async (req, res) => {
      try {
        const { jobId, filename } = req.params;
        const { getProjectAssets, getMimeType } = await import("./assetManager-DHW4N7CS.js");
...
`
        ...
        try {
          ...
          if (masterApiKey && masterApiKey.trim() !== "") {
            const masterKeyExists = apiKeys.some(
              (key) => key.apiProvider === masterProvider && key.apiKey === masterApiKey.trim() && key.apiModel === masterModel
            );
            if (!masterKeyExists) {
    ...
    const websocketManager2 = initializeWebSocketManager(server, redisClient);
    console.log("\u{1F4E1} WebSocket server initialized with app server.");
    if (process.env.NODE_ENV !== "test") {

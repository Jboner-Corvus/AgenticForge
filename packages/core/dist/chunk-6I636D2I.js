...
var Agent = class {
          ...
          for (let i = 0; i < config.LLM_PROVIDER_HIERARCHY.length; i++) {
            ...
            try {
              ...
              if (llmError instanceof LlmError && providerToTry === "qwen" && (llmError.message.includes(
                ...
                continue;
              } else if (llmError instanceof LlmError && (llmError.message.includes("API key not valid") || llmError.message.includes("API_KEY_INVALID") || llmError.message.includes("invalid api key"))) {
                this.log.error(
  ...
  parseLlmResponse(llmResponse, log) {
    ...
    const isIncomplete = llmResponse.trim().endsWith("...") || llmResponse.includes("ASSISTANT:") || llmResponse.includes("La r\xE9ponse de l'IA semble incompl\xE8te") || llmResponse.includes("The agent is thinking...") && !llmResponse.includes("Tool Call:") && !llmResponse.includes("{") || llmResponse.trim().length < 10;
  ...
  getNextFormFieldCommand() {
    const recentCommands = this.commandHistory.slice(-3);
    const lastTypeCommand = recentCommands.reverse().find(
      (cmd) => cmd.name === "playwright_type" && cmd.params
    );
    if (lastTypeCommand) {
      ...
      try {
        ...
        if (lockAge < 6e4) {
          ...
          try {
            ...
            try {
              process.kill(parseInt(existingPid), 0);
              getLoggerInstance().warn(`\u{1F52A} Force killing stubborn worker PID: ${existingPid}`);

...
import {
  ...
  getJobQueue,
  initializeWebServer
} from "./chunk-N7KB4RW6.js";
import {
  Agent,
  getMasterPrompt,
  initializeWorker,
  processJob
} from "./chunk-4VFNWZW4.js";
import "./chunk-SPW4FDKJ.js";
import {
  getAllTools
} from "./chunk-3BUXTMN2.js";
import {
  FinishToolSignal
} from "./chunk-CZQPSXPM.js";
import "./chunk-3GNHJQRZ.js";
import {
  SessionManager
} from "./chunk-3JUNTD77.js";
import "./chunk-TBECIF7Z.js";
...
import {
  LlmKeyErrorType
} from "./chunk-7Z7URPWJ.js";
import {
  disconnectRedis,
  getRedisClientInstance,
  setRedisClientInstance
} from "./chunk-ZLLQWWIP.js";
import {
  AppError,
  EnqueueTaskError,
  ...
  handleError,
  toolRegistry
} from "./chunk-TGZOLI7G.js";
import {
  getLogger,
  getLoggerInstance,
  resetLoggerForTesting
} from "./chunk-KU7FQUOD.js";
import {
  config,
  getConfig,
  loadConfig
} from "./chunk-7WLI2CKS.js";
import {
  __commonJS,
  __require,
  __toESM,
...
var serviceName = process.env.OTEL_SERVICE_NAME || "agentic-forge";
var collectorEndpoint = process.env.OTEL_COLLECTOR_ENDPOINT || "http://otel-collector:4317";
var sdk = new NodeSDK({
...
async function startApplication() {
  ...
  logger.info("D\xE9marrage de l'application AgenticForge...");
  ...
  try {
    logger.info("D\xE9marrage du serveur web...");
    ...
    logger.info("Serveur web AgenticForge d\xE9marr\xE9.");
  } catch (error) {
    logger.error(
      { error },
...
startApplication().catch((error) => {
  getLogger().error(
    error,
    "Erreur critique lors du d\xE9marrage de l'application."
  );
  process.exit(1);
});
...
async function enqueueTask(args) {
  ...
  try {
    ...
    log.info({ jobId: job.id }, "T\xE2che ajout\xE9e \xE0 la file d'attente.");
    return job.id;
  } catch (error) {
    const errDetails = getErrDetails(error);
    log.error("\xC9chec de l'ajout de la t\xE2che \xE0 la file d'attente.", {
      err: errDetails
    });
    throw new EnqueueTaskError(
      `L'ajout de la t\xE2che ${taskId} pour ${toolName} \xE0 la file d'attente a \xE9chou\xE9 : ${errDetails.message}`,
...
var ANSI_COLORS = {
  ...
  GREEN: "\x1B[32m",
  LIGHT_BLUE: "\x1B[94m",
  // Ajout pour une différenciation
  MAGENTA: "\x1B[35m",
  RED: "\x1B[31m",
  RESET: "\x1B[0m",
  YELLOW: "\x1B[33m"
};
var WEBHOOK_SIGNATURE_HEADER = "X-Webhook-Signature-256";
var WEBHOOK_SECRET_ENV_VAR = "WEBHOOK_SECRET";
var ERROR_STACK_TRACE_MAX_LENGTH = 250;
var DEFAULT_BULLMQ_JOB_OPTIONS = {
  attempts: 3,
  backoff: { delay: 5e3, type: "exponential" },
  removeOnComplete: { age: 24 * 3600, count: 1e3 },
  // Conserver pendant 24 heures
  removeOnFail: { age: 7 * 24 * 3600, count: 5e3 }
  // Conserver les échecs pendant 7 jours
};
var TASK_QUEUE_NAME = "async-tasks";
var DEAD_LETTER_QUEUE_NAME = "dead-letter-tasks";
var DEFAULT_PING_OPTIONS = {
  enabled: true,
  intervalMs: 15e3,
  // Augmenté pour moins de verbosité par défaut
  logLevel: "debug"
  // 'debug' est un LogLevel valide pour pino et FastMCP
};
var DEFAULT_HEALTH_CHECK_OPTIONS = {
  enabled: true,
  message: "ok",
  path: "/health",
  // Maintenu cohérent avec la configuration existante
  status: 200
};
...
async function runQualityGate(_ctx) {
  const logger = getLogger();
  ...
  const checks = [
    {
      ...
      command: "pnpm",
      cwd: path6.resolve(__dirname2, "../../ui"),
      name: "TypeCheck UI"
    },
    {
      args: ["exec", "tsc", "--noEmit"],
      command: "pnpm",
      cwd: path6.resolve(__dirname2, ".."),
      name: "TypeCheck Core"
    },
    {
      args: ["lint"],
...
--- Running: ${check.name} ---`);
    try {
      const { stderr, stdout } = await execa(check.command, check.args, {
        cwd: check.cwd,
        reject: false
        // Ne pas rejeter la promesse en cas d'erreur
      });
...
init_esm_shims();
function isValidHttpUrl(urlString, context) {
  if (!urlString) {
    return false;
  }
  try {
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") {
      if (context) {
        getLoggerInstance().warn(
          { context, reason: "Protocole non support\xE9", url: urlString },
    ...
    if (context) {
      getLoggerInstance().warn(
        ...
        "Format d'URL invalide d\xE9tect\xE9."
      );
    }
    return false;
  }
}
function validateApiKey(apiKey, expectedApiKey) {

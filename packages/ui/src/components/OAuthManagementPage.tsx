...
export const OAuthManagementPage = memo(() => {
  ...
  const initializeSessionAndMessages = useCombinedStore(
  ...
  const handleSaveToken = async () => {
    if (bearerToken.trim()) {
      ...
      setTokenStatus(true);
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [INFO] Bearer token saved successfully - Loading sessions`,
      );
      ...
      try {
        ...
        addDebugLog(
          `[${new Date().toLocaleTimeString()}] [INFO] Sessions loaded successfully - Redirecting to chat`,
        );
      } catch (error) {
        console.error('Failed to load sessions after authentication:', error);
        addDebugLog(
          `[${new Date().toLocaleTimeString()}] [ERROR] Failed to load sessions: ${error}`,
        );
      }
  ...
  useEffect(() => {
      ...
      try {
        ...
        try {
          ...
          if (response.ok) {
            ...
            addDebugLog(
              `[${new Date().toLocaleTimeString()}] [INFO] Qwen connection status: ${hasQwenToken ? 'connected' : 'disconnected'}`,
            );
          } else {
            addDebugLog(
              `[${new Date().toLocaleTimeString()}] [WARNING] Qwen status API returned: ${response.status}`,
            );
          }
        } catch (error) {
          console.error('Error checking Qwen connection status:', error);
          addDebugLog(
            `[${new Date().toLocaleTimeString()}] [ERROR] Qwen connection check failed: ${error instanceof Error ? error.message : String(error)}`,
          );
        }

        // Log the OAuth status check
        addDebugLog(
          `[${new Date().toLocaleTimeString()}] [INFO] Checking OAuth status - GitHub: ${hasGitHubToken}, Google: ${hasGoogleToken}, Twitter: ${isTwitterConnected}, Qwen: ${hasQwenToken}`,
        );
        ...
        if (authHeader) {
          addDebugLog(
            `[${new Date().toLocaleTimeString()}] [INFO] Backend Bearer Token found in localStorage: ${authHeader.substring(0, 20)}...`,
          );
        } else {
          ...
          if (tokenCookie) {
            ...
            addDebugLog(
              `[${new Date().toLocaleTimeString()}] [INFO] Backend Bearer Token found in cookies: ${token.substring(0, 20)}...`,
            );
          } else {
            addDebugLog(
              `[${new Date().toLocaleTimeString()}] [INFO] No backend Bearer Token found`,
            );
          }
        }
        ...
        addDebugLog(
          `[${new Date().toLocaleTimeString()}] [ERROR] Error checking OAuth status: ${error instanceof Error ? error.message : String(error)}`,
        );
        setIsCheckingStatus(false);
      }
    };
    ...
    const timeoutId = setTimeout(() => {
      setIsCheckingStatus(false);
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [WARNING] OAuth status check timeout - forced unlock`,
      );
    }, 10000);
  ...
  const handleGitHubLogin = () => {
    try {
      ...
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [INFO] Initiating GitHub OAuth flow`,
      );
      ...
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [ERROR] Error initiating GitHub OAuth: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  const handleGoogleLogin = () => {
    try {
      ...
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [INFO] Initiating Google OAuth flow`,
      );
      ...
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [ERROR] Error initiating Google OAuth: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  const handleTwitterLogin = () => {
    try {
      ...
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [INFO] Initiating Twitter OAuth flow`,
      );
      ...
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [ERROR] Error initiating Twitter OAuth: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  const handleGitHubLogout = () => {
    try {
      ...
      document.cookie =
        'agenticforge_jwt=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
      ...
      // Add debug log
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [INFO] GitHub OAuth token removed.`,
      );
    } catch (error) {
      console.error('Error removing GitHub OAuth token:', error);
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [ERROR] Error removing GitHub OAuth token: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  const handleGoogleLogout = () => {
    try {
      ...
      document.cookie =
        'agenticforge_google_token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
      ...
      // Add debug log
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [INFO] Google OAuth token removed.`,
      );
    } catch (error) {
      console.error('Error removing Google OAuth token:', error);
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [ERROR] Error removing Google OAuth token: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  const handleTwitterLogout = () => {
    try {
      ...
      document.cookie =
        'agenticforge_twitter_token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
      ...
      // Add debug log
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [INFO] Twitter OAuth token removed.`,
      );
    } catch (error) {
      console.error('Error removing Twitter OAuth token:', error);
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [ERROR] Error removing Twitter OAuth token: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  const handleQwenLogin = async () => {
    try {
      ...
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [INFO] Checking Qwen.AI Chat 2000 credentials`,
      );
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.accessToken) {
            ...
            addDebugLog(
              `[${new Date().toLocaleTimeString()}] [INFO] Qwen.AI Chat 2000 Access Token found: ${data.accessToken.substring(0, 20)}...`,
            );
...
The full token has been copied to your clipboard.`);
            ...
            addDebugLog(
              `[${new Date().toLocaleTimeString()}] [INFO] No Qwen.AI Chat 2000 credentials found, initiating OAuth flow`,
            );
          ...
          addDebugLog(
            `[${new Date().toLocaleTimeString()}] [INFO] API error checking credentials, initiating OAuth flow`,
          );
          window.location.href = '/api/auth/qwen';
        }
      } catch (error) {
        ...
        addDebugLog(
          `[${new Date().toLocaleTimeString()}] [ERROR] Error checking Qwen credentials: ${error instanceof Error ? error.message : String(error)}, initiating OAuth flow`,
        );
        window.location.href = '/api/auth/qwen';
      }
    } catch (error) {
      ...
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [ERROR] Error checking Qwen.AI Chat 2000 credentials: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  const handleQwenLogout = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Add debug log
        addDebugLog(
          `[${new Date().toLocaleTimeString()}] [INFO] Qwen.AI Chat 2000 OAuth token removed.`,
        );
      } else {
      ...
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [ERROR] Error removing Qwen.AI Chat 2000 OAuth token: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
          ...
          <CardContent className="space-y-6 pt-6 pb-8 max-h-[calc(100vh-200px)] overflow-y-auto">
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-400">
                To use the agent, you need to provide a valid authentication
                token. This token is used to authenticate your requests to the
                backend API.
              </p>
            ...
            >
              <Alert
                variant="default"
                className="border-2 border-blue-700/50 bg-blue-900/20 backdrop-blur-sm"
              >
                <AlertTriangle className="h-5 w-5 text-blue-400" />
                <AlertTitle className="font-bold text-blue-300">
                  Authentication Setup
                </AlertTitle>
                <AlertDescription className="text-blue-400/80">
                  <p className="mb-2">
                    To use the agent, you need to authenticate with one of the
                    supported providers below. After connecting, your
                    authentication token will be automatically set up for use
                    with the agent.
                  </p>
                  <p>
                    If you prefer to use a manual token, you can set it up in
                    the LLM API Key Management section.
                  </p>
                </AlertDescription>
              </Alert>
            </motion.div>
            ...
            >
              <Alert
                variant="default"
                className="border-2 border-amber-700/50 bg-amber-900/20 backdrop-blur-sm"
              >
                <AlertTriangle className="h-5 w-5 text-amber-400" />
                <AlertTitle className="font-bold text-amber-300">
                  {translations.securityWarning}
                </AlertTitle>
                  ...
                  >
                    <div>
                      <p className="font-medium text-white">
                        {translations.githubConnectionStatus}
                      </p>
                      <p className="text-sm text-gray-400">
                        {isGitHubConnected
              ...
              >
                ...
                {isCheckingStatus ? (
                  ...
                  >
                    <div>
                      <p className="font-medium text-white">
                        {translations.googleConnectionStatus}
                      </p>
                      <p className="text-sm text-gray-400">
                        {isGoogleConnected
              ...
              >
                ...
                {isCheckingStatus ? (
                  ...
                  >
                    <div>
                      <p className="font-medium text-white">
                        {translations.twitterConnectionStatus}
                      </p>
                      <p className="text-sm text-gray-400">
                        {isTwitterConnected
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-purple-400/80 mt-1">
                          Advanced AI interface with Qwen models - 2000 free
                          requests per day
                        </p>
                        {isQwenConnected && (
                    ...
                    {isQwenConnected && (
                      <div className="mt-4 pt-4 border-t border-purple-800/50">
                        <Alert
                          variant="default"
                          className="mb-4 border-2 border-amber-700/50 bg-amber-900/20 backdrop-blur-sm"
                        >
                          <AlertTriangle className="h-5 w-5 text-amber-400" />
                          <AlertTitle className="font-bold text-amber-300">
                            Note importante
                          ...
                          <AlertDescription className="text-amber-400/80">
                            Pour maintenir la validité de la clé, ne rouvrez
                            jamais l'interface CLI de Qwen après l'avoir
                            obtenue.
                          </AlertDescription>
                        </Alert>
                        <div className="flex items-center justify-between">
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2 text-xs">
                          ...
                          <div className="flex items-center">
                            ...
                            <span
                              className={`ml-1 ${qwenTokenStatus.isValid ? 'text-green-400' : 'text-red-400'}`}
                            >
                  ...
                  {debugLogs.length > 0 ? (
                    <ul className="space-y-2">
                      {debugLogs.map((log: string, index: number) => (
                        <li
                          key={index}
                          className="text-xs font-mono text-gray-400 p-2 bg-gray-900/50 rounded border border-gray-700"
                        >
                          {log}
                        </li>
                      ))}

    ...
    if (!canvasContent) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
          ...
          <p>Le contenu de l'agent apparaîtra ici.</p>
        </div>
      );
    }
    switch (canvasType) {
      case 'html':
        return <iframe srcDoc={canvasContent} title="Agent HTML Output" className="w-full h-full border-0" sandbox="allow-scripts" />;
      case 'markdown':
        return <div className="p-4 prose dark:prose-invert"><ReactMarkdown remarkPlugins={[remarkGfm]}>{canvasContent}</ReactMarkdown></div>;
      case 'url':
        return <iframe src={canvasContent} title="Agent URL Output" className="w-full h-full border-0" sandbox="allow-scripts allow-same-origin" />;
      case 'text':
      default:
        return <pre className="p-4 text-sm whitespace-pre-wrap">{canvasContent}</pre>;
    }
  };
  ...
  return (
    ...
    >
      <header className="flex items-center justify-between p-2 flex-shrink-0">
        <h2 className="text-lg font-semibold ml-2">Agent Output Canvas</h2>

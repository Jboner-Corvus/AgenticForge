...
export const DraggableControlPanel = memo(() => {
  ...
  // Draggable hook
  const { position, handleDragStart, elementRef } = useDraggable({
    x: 20,
    y: 20,
  });
  ...
  const displayedSessions = showAllSessions
    ? sessions
    : sessions.slice(0, MAX_INITIAL_SESSIONS);
  ...
  return (
    ...
    >
      <div
        className="absolute top-0 left-0 w-full h-8 cursor-move bg-secondary/30 flex items-center justify-center z-10"
        onMouseDown={handleDragStart}
      >
        <Move className="h-4 w-4 text-muted-foreground" />
      </div>

      <aside className="p-4 bg-gradient-to-b from-background to-secondary/30 border border-border relative pt-8 shadow-lg">
            ...
            <Card className="shadow-md hover:shadow-lg transition-shadow duration-300 border-2 border-yellow-500">
              ...
              <CardContent className="space-y-4 pt-4">
                <p className="text-sm text-yellow-800">
                  ...
                  Visit the OAuth Management page to configure your
                  authentication.
                </p>
                <Button
            ...
            <CardContent className="space-y-4 pt-4">
              ...
              <div className="flex justify-between items-center p-2 rounded hover:bg-accent transition-all duration-200 hover:scale-105">
                ...
                >
                  ...
                  {translations.connectionStatus}
                </Label>
                <Badge
                  id="connection-status"
                  variant={serverHealthy ? 'success' : 'destructive'}
                >
              ...
              <div className="flex justify-between items-center p-2 rounded hover:bg-accent transition-all duration-200 hover:scale-105">
                <Label
                  htmlFor="browser-status"
                  className="text-sm flex items-center"
                >
                  <Hammer className="mr-2 h-4 w-4 text-purple-500" />
                  Browser Status
                </Label>
                <span
                  id="browser-status"
              ...
              <div className="flex justify-between items-center p-2 rounded hover:bg-accent transition-all duration-200 hover:scale-105">
                <Label
                  htmlFor="tools-detected"
                  className="text-sm flex items-center"
                >
                  <Hammer className="mr-2 h-4 w-4 text-orange-500" />
                  {translations.toolsDetected}
                </Label>
                <span
                  id="tools-detected"
                  className="text-sm text-muted-foreground"
          ...
          <Card className="shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
            <CardHeader className="bg-gradient-to-r from-purple-500 to-pink-600 text-white rounded-t-md">
              <CardTitle className="flex items-center text-lg">
                ...
                {translations.historyAndActions}
              </CardTitle>
            </CardHeader>
                    ...
                    <div className="space-y-2">
                      {displayedSessions.map((session: unknown) => (
                        <div
                          ...
                          className="flex items-center justify-between p-2 border border-border rounded-md hover:bg-accent transition-all duration-200 hover:scale-105 transform"
                        >
                          <span
                            className="text-sm truncate"
                            title={(session as any).name}
                          >
                            {(session as any).name.length > 20
                              ? `${(session as any).name.substring(0, 20)}...`
                              : (session as any).name}
                            {(session as any).id === activeSessionId && (
                          ...
                          <div className="flex space-x-1">
                            <TooltipProvider>
                                ...
                                <TooltipTrigger asChild>
                                  <div className="transition-all duration-200 transform hover:scale-110">
                                    <Button
                                      ...
                                      variant="ghost"
                                      onClick={() =>
                                        handleLoadSession((session as any).id)
                                      }
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div className="transition-all duration-200 transform hover:scale-110">
                                    <Button
                                      ...
                                      variant="ghost"
                                      onClick={() =>
                                        handleOpenRenameModal(session as any)
                                      }
                                ...
                                <TooltipContent>
                                  <p>{translations.renameSession}</p>
                                </TooltipContent>
                              </Tooltip>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div className="transition-all duration-200 transform hover:scale-110">
                                    <Button
                                      ...
                                      variant="ghost"
                                      onClick={() =>
                                        handleDeleteSession((session as any).id)
                                      }
                                ...
                                <TooltipContent>
                                  <p>{translations.deleteSession}</p>
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          </div>
      ...
      <Modal
        ...
        title={translations.renameSession}
      >
        <Input
      ...
      <Modal
        ...
        title={translations.saveCurrentSession}
      >
        <Label htmlFor="saveSessionInput" className="sr-only">
          {translations.sessionNamePlaceholder}
        </Label>
        <Input
        ...
        <div className="flex justify-end space-x-2 mt-4">
          ...
          <Button onClick={handleConfirmSaveSession}>
            {translations.save}
          </Button>
        </div>
      </Modal>
      ...
      <Modal
        ...
        title={translations.confirmDeletion}
      >

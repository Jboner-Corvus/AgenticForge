...
const SubAgentCLIView: React.FC<SubAgentCLIViewProps> = ({ jobId }) => {
  ...
  useEffect(() => {
    ...
    eventSource.onerror = () => {
      setOutput((prevOutput) => [
        ...prevOutput,
        { type: 'error', content: 'Connection to stream lost.' },
      ]);
      eventSource.close();
    };
  ...
  return (
    <div
      className="bg-black text-white font-mono text-sm rounded-lg p-4 h-96 overflow-y-auto"
      ref={cliOutputRef}
    >
      <div className="border-b border-gray-700 mb-2 pb-2">
        <h3 className="text-lg font-bold text-green-400">
          GForce Agent Delegation
        </h3>
        <p className="text-xs text-gray-500">
          Streaming output for Job ID: {jobId}
        </p>
      </div>

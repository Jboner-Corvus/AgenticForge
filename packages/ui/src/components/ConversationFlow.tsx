...
export const ConversationFlow: React.FC<{ messages: any[] }> = ({
  messages,
}) => {
  return (
...
const ConversationFlowContent: React.FC<{ messages: any[] }> = memo(
  ({ messages }) => {
        ...
        {displayedMessages.map((message, index) => (
          ...
          >
            ...
            <div className="flex flex-col items-center flex-shrink-0">
              <div
                className={`w-6 h-6 rounded-full ${getMessageTypeColor(message)} flex items-center justify-center`}
              >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium text-white ${getMessageTypeColor(message)}`}
                >
...
const getMessageTypeName = (message: ChatMessage): string => {
  switch (message.type) {
    ...
    case 'agent_thought':
      return 'Réflexion';
    case 'tool_call':
      return "Appel d'outil";
    case 'tool_result':
      return "Résultat d'outil";
    case 'agent_response':
      return 'Réponse';
    case 'error':
      return 'Erreur';

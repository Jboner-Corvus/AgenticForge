...
export const AuthErrorHandler: React.FC<AuthErrorHandlerProps> = ({
  ...
  const { setAuthToken, addDebugLog, initializeSessionAndMessages } =
    useCombinedStore();
  ...
  const { validateToken, getCurrentTokenStatus } = useAuthInterceptor({
    onAuthError: useCallback(
      (error: AuthError) => {
        setLastError(error);
        addDebugLog(
          `[${new Date().toLocaleTimeString()}] [AUTH_HANDLER] Erreur 401 détectée sur ${error.url}`,
        );
  ...
  const handleSaveToken = async () => {
    ...
    try {
      ...
      if (isValid) {
        ...
        addDebugLog(
          `[${new Date().toLocaleTimeString()}] [AUTH] Token validé et sauvegardé avec succès`,
        );
        ...
        try {
          ...
          addDebugLog(
            `[${new Date().toLocaleTimeString()}] [AUTH] Sessions rechargées avec succès`,
          );
        } catch (error) {
        ...
        setValidationStatus('error');
        addDebugLog(
          `[${new Date().toLocaleTimeString()}] [AUTH] Token invalide rejeté`,
        );
      }
    } catch (error) {
      setValidationStatus('error');
      addDebugLog(
        `[${new Date().toLocaleTimeString()}] [AUTH] Erreur validation: ${error}`,
      );
    } finally {
    ...
    >
      <div className="space-y-4">
        ...
        <p className="text-gray-300 text-center mb-6">
          Votre token d'authentification est{' '}
          {tokenStatus === 'missing' ? 'manquant' : 'invalide'}. Veuillez entrer
          un token valide pour continuer.
        </p>
          ...
          <AlertDescription className="text-gray-300">
            <div className="flex items-center justify-between">
              <span>
                ...
                {tokenStatus === 'unknown' &&
                  "Token présent mais erreur d'authentification"}
              </span>
          ...
          >
            ...
            <AlertDescription className="text-red-400 text-sm">
              <div className="space-y-1">
                ...
                <div>
                  <strong>Méthode:</strong> {lastError.method}
                </div>
                <div>
                  <strong>Heure:</strong>{' '}
        ...
        <div className="bg-blue-900/20 border border-blue-700/50 rounded-lg p-3">
          ...
          <ul className="text-sm text-blue-400 space-y-1">
            <li>
              ...
              <code className="bg-gray-800 px-1 rounded">.env</code> pour
              AUTH_TOKEN
            </li>

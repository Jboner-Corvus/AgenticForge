...
export interface LLMKeysState {
  ...
  addKey: (
    key: Omit<LLMKey, 'id' | 'createdAt' | 'updatedAt' | 'usageCount'>,
  ) => Promise<void>;
    ...
    (set, get) => ({
      ...
      fetchKeys: async () => {
        ...
        try {
          ...
          const rawKeys: LLMKey[] = rawBackendKeys
            .map((backendKey: BackendKey, index: number) => {
              ...
              if (backendKey.id && backendKey.providerId) {
                return {
                  ...
                  providerName:
                    backendKey.providerName || backendKey.providerId,
                  keyName: backendKey.keyName || `Key ${index + 1}`,
                  keyValue: backendKey.apiKey,
                  isEncrypted: backendKey.isEncrypted || false,
                  isActive:
                    backendKey.isActive !== undefined
                      ? backendKey.isActive
                      : !backendKey.isPermanentlyDisabled,
                  ...
                  lastUsed: backendKey.lastUsed
                    ? new Date(backendKey.lastUsed).toISOString()
                    : undefined,
                  ...
                  usageStats: backendKey.usageStats || {
                    ...
                    errorRate: backendKey.errorCount
                      ? backendKey.errorCount / (backendKey.usageCount || 1)
                      : 0,
                  },
              ...
              return {
                id:
                  backendKey.id ||
                  `${backendKey.apiProvider || 'unknown'}-${index}-${Date.now()}`,
                providerId: backendKey.apiProvider || 'unknown',
                providerName: backendKey.apiProvider || 'unknown',
                keyName: backendKey.apiModel
                  ? `${backendKey.apiProvider || 'unknown'} - ${backendKey.apiModel}`
                  : `Key ${index + 1}`,
                keyValue: backendKey.apiKey,
                isEncrypted: false,
          ...
          const providersCount = new Set(keys.map((key) => key.providerId))
            .size;
      ...
      addKey: async (keyData) => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to add key: ${response.status} ${response.statusText}`,
            );
          }
      ...
      updateKey: async (id, updates) => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to update key: ${response.status} ${response.statusText}`,
            );
          }
      ...
      testKey: async (id) => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to test key: ${response.status} ${response.statusText}`,
            );
          }
          ...
          if (result.valid) {
            ...
            console.log(
              `❌ Key test failed for ${key.keyName}: ${result.message || result.error || 'Unknown error'}`,
            );
          }
      ...
      syncWithRedis: async () => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to sync with Redis: ${response.status} ${response.statusText}`,
            );
          }
      ...
      importKeysFromRedis: async () => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to import keys from Redis: ${response.status} ${response.statusText}`,
            );
          }
          ...
          set({
            error:
              error instanceof Error
                ? error.message
                : 'Failed to import keys from Redis',
            isSyncing: false,
          });
        }
      },
      ...
      exportKeysToRedis: async () => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to export keys to Redis: ${response.status} ${response.statusText}`,
            );
          }
          ...
          set({
            error:
              error instanceof Error
                ? error.message
                : 'Failed to export keys to Redis',
            isSyncing: false,
          });
        }
      },
      ...
      cleanupDuplicates: async () => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to cleanup duplicates: ${response.status} ${response.statusText}`,
            );
          }
      ...
      toggleShowInactiveKeys: () =>
        set((state) => ({ showInactiveKeys: !state.showInactiveKeys })),
        ...
        set((state) => {
          ...
          if (keyIndex !== -1) {
            ...
            const totalResponseTime =
              usageStats.averageResponseTime * (usageStats.totalRequests - 1) +
              responseTime;
            usageStats.averageResponseTime =
              totalResponseTime / usageStats.totalRequests;

            // Update error rate
            usageStats.errorRate =
              usageStats.failedRequests / usageStats.totalRequests;

  ...
  devtools(
    (set, get) => ({
      ...
      setIsSettingsModalOpen: (isOpen: boolean) =>
        useUIStore.getState().setIsSettingsModalOpen(isOpen),
      setIsControlPanelVisible: (isVisible: boolean) =>
        useUIStore.getState().setIsControlPanelVisible(isVisible),
      setIsTodoListVisible: (isVisible: boolean) =>
        useUIStore.getState().setIsTodoListVisible(isVisible),
      toggleDebugLogVisibility: () =>
        useUIStore.getState().toggleDebugLogVisibility(),
      toggleDarkMode: () => useUIStore.getState().toggleDarkMode(),
      setIsProcessing: (isProcessing: boolean) =>
        useUIStore.getState().setIsProcessing(isProcessing),
      setAgentProgress: (progress: number) =>
        useUIStore.getState().setAgentProgress(progress),
      setMessageInputValue: (value: string) =>
        useUIStore.getState().setMessageInputValue(value),
      setAgentStatus: (status: string | null) =>
        useUIStore.getState().setAgentStatus(status),
      setToolStatus: (status: string) =>
        useUIStore.getState().setToolStatus(status),
      setBrowserStatus: (status: string) =>
        useUIStore.getState().setBrowserStatus(status),
      setServerHealthy: (healthy: boolean) =>
        useUIStore.getState().setServerHealthy(healthy),
      setTokenStatus: (status: boolean) =>
        useUIStore.getState().setTokenStatus(status),
      setToolCount: (count: number | string) =>
        useUIStore.getState().setToolCount(count),
      setToolCreationEnabled: (enabled: boolean) =>
        useUIStore.getState().setToolCreationEnabled(enabled),
      setCodeExecutionEnabled: (enabled: boolean) =>
        useUIStore.getState().setCodeExecutionEnabled(enabled),
      setAuthToken: (token: string | null) =>
        useUIStore.getState().setAuthToken(token),
      setJobId: (jobId: string | null) => useUIStore.getState().setJobId(jobId),
      setActiveCliJobId: (jobId: string | null) =>
        useUIStore.getState().setActiveCliJobId(jobId),
      ...
      setCanvasContent: (content: string) =>
        useCanvasStore.getState().setCanvasContent(content),
      setCanvasType: (type: unknown) =>
        useCanvasStore.getState().setCanvasType(type as any),
      setIsCanvasVisible: (isVisible: boolean) =>
        useCanvasStore.getState().setIsCanvasVisible(isVisible),
      setCanvasPinned: (isPinned: boolean) =>
        useCanvasStore.getState().setCanvasPinned(isPinned),
      setCanvasFullscreen: (isFullscreen: boolean) =>
        useCanvasStore.getState().setCanvasFullscreen(isFullscreen),
      setCanvasWidth: (width: number) =>
        useCanvasStore.getState().setCanvasWidth(width),
      clearCanvas: () => useCanvasStore.getState().clearCanvas(),
      resetCanvas: () => useCanvasStore.getState().resetCanvas(),
      toggleIsCanvasVisible: () =>
        useCanvasStore.getState().toggleIsCanvasVisible(),
      addCanvasToHistory: (title: string, content: string, type: unknown) =>
        useCanvasStore
          .getState()
          .addCanvasToHistory(title, content, type as any),
      navigateToCanvas: (index: number) =>
        useCanvasStore.getState().navigateToCanvas(index),
      removeCanvasFromHistory: (index: number) =>
        useCanvasStore.getState().removeCanvasFromHistory(index),
      ...
      setSessionStatus: (status: unknown) =>
        useSessionStore.getState().setSessionStatus(status as any),
      setMessages: (messages: unknown[]) =>
        useSessionStore.getState().setMessages(messages as any),
      setSessions: (sessions: unknown[]) =>
        useSessionStore.getState().setSessions(sessions as any),
      setActiveSessionId: (id: string | null) =>
        useSessionStore.getState().setActiveSessionId(id),
      addMessage: (message: unknown) =>
        useSessionStore.getState().addMessage(message as any),
      clearMessages: () => useSessionStore.getState().clearMessages(),
      saveSession: (name: string) =>
        useSessionStore.getState().saveSession(name),
      ...
      setIsLoadingSessions: (isLoading: boolean) =>
        useSessionStore.getState().setIsLoadingSessions(isLoading),
      setIsSavingSession: (isSaving: boolean) =>
        useSessionStore.getState().setIsSavingSession(isSaving),
      setIsDeletingSession: (isDeleting: boolean) =>
        useSessionStore.getState().setIsDeletingSession(isDeleting),
      setIsRenamingSession: (isRenaming: boolean) =>
        useSessionStore.getState().setIsRenamingSession(isRenaming),

      // LLM API Key Management
      addLlmApiKey: async (apiKey: LlmApiKey) => {
        set({ isAddingLlmApiKey: true });
        try {
          await addLlmApiKeyApi(
            apiKey.provider || '',
            apiKey.key || '',
            apiKey.baseUrl,
            apiKey.model,
      ...
      initializeSessionAndMessages: async () => {
        try {
          ...
          if (!authToken) {
            console.log(
              '🔐 [Combined Store] No auth token found, skipping all backend loading. User needs to authenticate first.',
            );
          ...
          console.log(
            '🔐 [Combined Store] Auth token found, proceeding with backend data loading...',
          );
      ...
      setIsRemovingLlmApiKey: (isRemovingLlmApiKey) =>
        set({ isRemovingLlmApiKey }),
